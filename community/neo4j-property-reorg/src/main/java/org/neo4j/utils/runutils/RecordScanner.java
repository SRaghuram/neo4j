...
import org.neo4j.propertystorereorg.PropertystoreReorgTool;
...
public class RecordScanner<RECORD> implements StoppableRunnable
{
    ...
    public void run( StoreDetails storeDetails )
    {
        run( parallel, storeDetails );
    }
    ...
    public void run(boolean parallel, StoreDetails storeDetails)
    {
        long start = System.currentTimeMillis();
        for (int i = 0; i < iteration; i++)
        {
            preProcess( storeDetails );
        	if (!parallel)
        		runSequential(storeDetails);
        	else
        		runParallel( storeDetails );
        	postProcess( storeDetails );
        }
    }
    
    public void runSequential(StoreDetails storeDetails)
    {
    	threadIndex.set(0);
        try
        {
            ...
            for ( RECORD record : store )
            {
                ...
                processor.process( (Abstract64BitRecord)record, storeDetails );
    ...
    public void runParallel( StoreDetails storeDetails )
    {
        ...
        public void run( StoreDetails storeDetails )
        {
            ...
            while (!done || !recordsQ[threadId].isEmpty())
            {
            	try {
            		...
            		if (record != null)
            			processor.process( (PrimitiveRecord)record, storeDetails );
            	} catch (Exception ie)
            	{
    ...
    public void preProcess( StoreDetails storeDetails )
    {
        if (commands.length > 0 && commands[0] != null)
            commands[0].runCommand(storeDetails, this);      
    }

    public void postProcess( StoreDetails storeDetails )
    {
        if (commands.length > 1 && commands[1] != null)
            commands[1].runCommand(storeDetails, this);
