...
public class NodeLabelCountProcess implements RecordProcessor<Abstract64BitRecord> {
    ...
    public NodeLabelCountProcess( NodeStore store, StoreDetails storeDetails ) {
        this.store = store;
        labelCount = new long[(int)storeDetails.storeAccess.getLabelNameStore().getHighestPossibleIdInUse()+1];
        labelCountSave = new long[(int)storeDetails.storeAccess.getLabelNameStore().getHighestPossibleIdInUse()+1];
        labelOrderIndex = new long[(int)storeDetails.storeAccess.getLabelNameStore().getHighestPossibleIdInUse()+1];
    }
    ...
    public void process( Abstract64BitRecord record, StoreDetails storeDetails )
    {
        ...
        if (record instanceof NodeRecord)
        {
            ...
            Set<Long> nodeLabels = NodeLabelReader.getListOfLabels(node, storeDetails.storeAccess );
    ...
    public void close(StoreDetails storeDetails)
    {
        /*for (int i = 0; i < labelCount.length; i++)
        {
            System.out.println("["+i+"] ["+ 
                    (i == 0 ? "No Labels" : storeDetails.storeAccess.getRawNeoStores().getLabelTokenStore().getToken( i ).name())
