...
public class PropertyAndNodeIndexedCheck implements RecordCheck<NodeRecord, ConsistencyReport.NodeConsistencyReport>
{
    ...
    {
        List<PropertyRecord> properties = (List<PropertyRecord>)propertyReader.getPropertyRecordChain( record );
        PropertyCache.saveProperties(properties);
        if (indexes != null)
            checkIndexToLabels(record, engine, records, properties);
        checkProperty(record, engine, records, properties);
    }   
    ...
    private void checkIndexToLabels( NodeRecord record,
            CheckerEngine<NodeRecord, ConsistencyReport.NodeConsistencyReport> engine,
            RecordAccess records,
            List<PropertyRecord> propertyRecs)
    {
        ...
        for ( IndexRule indexRule : indexes.rules() )
        {
            ...
            if (properties == null)
                properties = propertyReader.propertyBlocks( propertyRecs );//propertyReader.propertyBlocks( record );
