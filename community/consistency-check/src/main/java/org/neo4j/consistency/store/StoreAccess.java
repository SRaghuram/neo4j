...
public class StoreAccess extends org.neo4j.kernel.impl.store.StoreAccess
{
    ...
    public StoreAccess( org.neo4j.kernel.impl.store.StoreAccess nativeStores )
    {
        super( nativeStores.getRawNeoStore() );
    ...
    public class AccessStats
    {
        ...
        public String toString()
        {
            ...
            int scatterIndex = 0;
            if ( randomReadsEst > 0  && reads > 0)
                scatterIndex  = (int)((randomReadsEst*100)/reads);
            buf.append( "[ScatterIndex: "+ scatterIndex + " %]" );
            if (scatterIndex > 0.5)
                buf.append(" [Property Store reorgization is recommended for optimal performance]");
            return buf.toString();
        }
