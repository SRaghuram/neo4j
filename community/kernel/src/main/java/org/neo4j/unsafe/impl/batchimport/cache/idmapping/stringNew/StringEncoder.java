...
/**
 * Encodes String into a long with very small chance of collision, i.e. two different Strings encoded into the same long value.
 *
...
public class StringEncoder implements Encoder
{
    ...
    @Override
    public long encode( Object s, int version )
    {
        int[] val;
        switch ( version )
        {
        case 0:
            val = encodeInt( wrap( s ), 0 );
            break;
        case 1:
            val = encodeInt(  wrap( s ), 1 );
            break;
        case 2:
            val = encodeInt(  wrap( s ), 2 );
            break;
        default:
            val = encodeInt(  wrap( s ), 0 );
    ...
    private int[] encodeInt( FlyweightString s, int version )
    {
        ...
        reMap( bytes, inputLength, version );
        // encode
    ...
    private int[] encodeInt( String s, int version )
    {
        // construct bytes from string
        int inputLength = s.length( );
        byte[] bytes = new byte[inputLength];
    ...
    private void reMap( byte[] bytes, int inputLength, int version )
    {
        for ( int i = 0; i < inputLength; i++ )
        {
            int byteIndex = ( bytes[i] + version * 8 ) % 127;
            if ( reMap[byteIndex] == -1 )
            {
                reMap[byteIndex] = (byte) ( numChars++ % 256 );
            }
            bytes[i] = reMap[byteIndex];
        }
    }
