...
public class DefaultIdGeneratorFactory implements IdGeneratorFactory
{
    ...
    public IdGenerator open( File fileName, int grabSize, IdType idType, Supplier<Long> highId, long maxId )
    {
        ...
        IdGenerator generator = instantiate( fs, fileName, grabSize, maxId, idTypeConfiguration.allowAggressiveReuse(), 
                idTypeConfiguration.allowDoubleRecordUnits(),  highId );
        generators.put( idType, generator );
        return generator;
    }

    protected IdGenerator instantiate( FileSystemAbstraction fs, File fileName, int grabSize, long maxValue,
            boolean aggressiveReuse, boolean allowDoubleRecordUnits, Supplier<Long> highId )
    {
        return new IdGeneratorImpl( fs, fileName, grabSize, maxValue, aggressiveReuse, allowDoubleRecordUnits, highId );
    }
