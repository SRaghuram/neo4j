  ...
  Describe "Get-Java" {
    ...
    Context "Server Invoke - Enterprise v4.0" {
      ...
      It "should have main class of com.neo4j.server.enterprise.EnterpriseEntryPoint" {
        $resultArgs | Should Match ([regex]::Escape(' com.neo4j.server.enterprise.EnterpriseEntryPoint'))
      }
    }
    ...
    Context "Utility Invoke - Should provide parallel gc option" {
      $serverObject = global:New-MockNeo4jInstall -ServerVersion '99.99' -ServerType 'Community'

      $result = Get-Java -ForUtility -StartingClass 'someclass' -Neo4jServer $serverObject -ErrorAction Stop
      $resultArgs = ($result.args -join ' ')

      It "should have parallel gc java option" {
        $resultArgs | Should Match ([regex]::Escape('-XX:+UseParallelGC'))
      }
    }
