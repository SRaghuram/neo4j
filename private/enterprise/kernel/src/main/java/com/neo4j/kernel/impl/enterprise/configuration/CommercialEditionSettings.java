...
public class CommercialEditionSettings implements SettingsDeclaration
{
    ...
    public static final Setting<List<IdType>> idTypesToReuse =
            newBuilder( "dbms.ids.reuse.types.override", listOf( ofPartialEnum( IdType.NODE, IdType.RELATIONSHIP ) ),
                    List.of( IdType.NODE, IdType.RELATIONSHIP ) ).build();

    @Description( "The maximum number of databases." )
    public static final Setting<Long> maxNumberOfDatabases = newBuilder( "dbms.max_databases", LONG, 100L ).build();
