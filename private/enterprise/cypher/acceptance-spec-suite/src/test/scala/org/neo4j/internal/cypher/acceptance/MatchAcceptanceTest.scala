...
class MatchAcceptanceTest extends ExecutionEngineFunSuite with QueryStatisticsTestSupport with CypherComparisonSupport {
  ...
  test("should return shortest path unbound length") {
    ...
    executeWith(Configs.InterpretedAndSlottedAndMorsel, "match p = shortestPath((a {name:'A'})-[*]-(b {name:'B'})) return p").toList
  }
  ...
  test("should not traverse same relationship twice in shortest path") {
    ...
    // when
    val result = executeWith(Configs.InterpretedAndSlottedAndMorsel, "MATCH (a{name:'A'}), (b{name:'B'}) MATCH p=allShortestPaths((a)-[:KNOWS|KNOWS*]->(b)) RETURN p").
      toList
  ...
  test("shortest Path Direction Respected") {
    ...
    val b = createNode()
    relate(a, b)
    val result = executeWith(Configs.InterpretedAndSlottedAndMorsel, "match (a), (b) where id(a) = 0 and id(b) = 1 match p=shortestPath((b)<-[*]-(a)) return p").toList.head("p").asInstanceOf[Path]
  ...
  test("should return shortest paths when only one side is bound") {
    ...
    val result = executeWith(Configs.InterpretedAndSlottedAndMorsel, "match (a:A) match p = shortestPath( (a)-[*]->(b:B) ) return p").toList.head("p").asInstanceOf[Path]
  ...
  test("should be able to use index hints with inequality/range predicates") {
    ...
    // when
    val result = executeWith(Configs.InterpretedAndSlottedAndMorsel, "MATCH (n:Person)-->() USING INDEX n:Person(name) WHERE n.name > 'Jac' RETURN n")
  ...
  test("should not fail if asking for a non existent node id with WHERE") {
    executeWith(Configs.InterpretedAndSlottedAndMorsel, "match (n) where id(n) in [0,1] return n")
      .toList
  ...
  test("should handle queries that cant be index seek solved because expressions lack dependencies") {
    ...
    // when
    val result = executeWith(Configs.InterpretedAndSlottedAndMorsel, "match (a:Label)-->(b:Label) where a.property = b.property return a, b")
  ...
  test("should use index for property existence queries (with Exists & OPTIONAL MATCH) when asked for it") {
    ...
    // when
    val result = executeWith(Configs.InterpretedAndSlottedAndMorsel, "OPTIONAL MATCH (n:User) USING INDEX n:User(email) WHERE exists(n.email) RETURN n")
  ...
  test("should use index for property existence queries (with IS NOT NULL & MATCH) when asked for it") {
    ...
    // when
    val result = executeWith(Configs.InterpretedAndSlottedAndMorsel, "MATCH (n:User) USING INDEX n:User(email) WHERE n.email IS NOT NULL RETURN n")
  ...
  test("should use index for property existence queries (with IS NOT NULL & OPTIONAL MATCH) when asked for it") {
    ...
    // when
    val result = executeWith(Configs.InterpretedAndSlottedAndMorsel, "OPTIONAL MATCH (n:User) USING INDEX n:User(email) WHERE n.email IS NOT NULL RETURN n")
  ...
  test("should use the index for property existence queries when cardinality prefers it") {
    ...
    // when
    val result = executeWith(Configs.InterpretedAndSlottedAndMorsel, "MATCH (n:User) WHERE exists(n.email) RETURN n")
  ...
  test("should not use the index for property existence queries when property value predicate exists") {
    ...
    // when
    val result = executeWith(Configs.InterpretedAndSlottedAndMorsel, "MATCH (n:User) WHERE exists(n.email) AND n.email = 'me@mine' RETURN n")
  ...
  test("should handle distinct, variable length and relationship predicate") {
    ...
    // When
    val res = executeWith(Configs.InterpretedAndSlottedAndMorsel, "MATCH (n)-[r:T*2]->() WHERE last(r).roles = 'NEO' RETURN DISTINCT n")
  ...
  test("should handle distinct collect where the path is needed") {
    ...
    // When
    val result = executeWith(Configs.InterpretedAndSlottedAndMorsel, "MATCH p=(n)-[*0..3]-() RETURN size(COLLECT(DISTINCT p)) AS size")
  ...
  test("Should handle multiple labels with OR and AND") {
    ...
    val result = executeWith(Configs.InterpretedAndSlottedAndMorsel, "MATCH (a) WHERE (a:A AND a:B) OR (a:A AND a:C) RETURN a")
  ...
  test("should not touch the database when for impossible anded predicates") {
    ...
    // When
    val result = executeWith(Configs.InterpretedAndSlottedAndMorsel, "PROFILE MATCH (n) WHERE 1 = 0 AND 5 > 1 RETURN n")
  ...
  test("should not touch the database when for impossible or'd predicates") {
    ...
    // When
    val result = executeWith(Configs.InterpretedAndSlottedAndMorsel, "PROFILE MATCH (n) WHERE 1 = 0 OR 1 > 5 RETURN n")
  ...
  test("Problem with index seek gh #10387") {
    ...
    val res = executeWith(Configs.InterpretedAndSlotted + TestConfiguration("4.0 runtime=compiled debug=generate_java_source"), q)
