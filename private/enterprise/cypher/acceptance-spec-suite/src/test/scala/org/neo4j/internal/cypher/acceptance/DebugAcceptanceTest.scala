  ...
  test("CYPHER DEBUG=rawCardinalities should output estimated rows with float precision") {
    ...
    val expected =
      """
        ...
        |+-------------------+--------------------+---------------------------+------------+
        || +CartesianProduct |                    |              400.0 (10.0) | n.prop ASC |
        || |\                +--------------------+---------------------------+------------+
        || | +AllNodesScan   | m                  | 20.0 (3.1622776601683795) |            |
        || |                 +--------------------+---------------------------+------------+
        || +Sort             | `n.prop` ASC       | 20.0 (3.1622776601683795) | n.prop ASC |
        || |                 +--------------------+---------------------------+------------+
        || +Projection       | n.prop AS `n.prop` |               20.0 (20.0) |            |
        || |                 +--------------------+---------------------------+------------+
        || +AllNodesScan     | n                  |               20.0 (20.0) |            |
        |+-------------------+--------------------+---------------------------+------------+
        |
