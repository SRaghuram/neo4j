...
class ProfilerAcceptanceTest extends ExecutionEngineFunSuite with CreateTempFileTestSupport with CypherComparisonSupport {
  ...
  test("match (n) where (n)-[:FOO]->() return *") {
    ...
    //WHEN
    val result = profileWithExecute(Configs.InterpretedAndSlotted, "match (n) where (n)-[:FOO]->() return *")
  ...
  test("match (n) where not (n)-[:FOO]->() return *") {
    ...
    //WHEN
    val result = profileWithExecute(Configs.InterpretedAndSlotted, "match (n) where not (n)-[:FOO]->() return *")
  ...
  test("should not have a problem profiling empty results") {
    val result = profileWithExecute(Configs.InterpretedAndSlotted, "MATCH (n) WHERE (n)-->() RETURN n")
