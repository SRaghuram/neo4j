...
class IndexWithValuesAcceptanceTest extends ExecutionEngineFunSuite with QueryStatisticsTestSupport with CypherComparisonSupport {
  ...
  test("should pass cached property through distinct when it's not part of a distinct column - single node") {
    val result = executeWith(Configs.InterpretedAndSlotted + Configs.Version3_4, "PROFILE MATCH (n:Awesome {prop1: 40}) WITH DISTINCT n MATCH (n)-[:R]->() RETURN n.prop1", executeBefore = createSomeNodes)
  ...
  test("should pass cached property through distinct when it's not part of a distinct column - multiple nodes") {
    ...
    val result = executeWith(Configs.InterpretedAndSlotted + Configs.Version3_4, "PROFILE MATCH (n:Awesome {prop1: 40})-[:R]-(b) WITH DISTINCT n, b MATCH (b)-[:R2]->() RETURN n.prop1", executeBefore = createSomeNodes)
  ...
  test("should not pass cached properties through aggregations") {
    val config = Configs.InterpretedAndSlotted + Configs.Version3_4 - Configs.Compiled - Configs.Cost2_3
  ...
  test("index-backed property values should be updated on property write with generic set +=") {
    ...
    val result = executeWith(Configs.InterpretedAndSlotted /\ Configs.Version3_5 + Configs.Rule3_1, query)
    assertIndexSeekWithValues(result)
