...
class UsingAcceptanceTest extends ExecutionEngineFunSuite with RunWithConfigTestSupport with CypherComparisonSupport {
  ...
  test("fail if using a variable with label not used in match") {
    ...
    // WHEN
    failWithError(Configs.All + Configs.Morsel, "match n-->() using index n:Person(name) where n.name = 'kabam' return n",
  ...
  test("fail when equality checks are done with OR") {
    ...
    // WHEN
    failWithError(Configs.All + Configs.Morsel, "match n-->() using index n:Person(name) where n.name = 'kabam' OR n.name = 'kaboom' return n",
  ...
  test("should handle join hint on the start node of a single hop pattern") {
    ...
    val result = executeWith(Configs.Version4_0 + Configs.Version3_4, query,
  ...
  test("should accept hint on composite index") {
    ...
    val query =
      """ MATCH (f:Foo)
        ...
        | RETURN f
      """.stripMargin
    val result = executeWith(Configs.Version4_0 + Configs.Version3_4 - Configs.Compiled, query,
                             planComparisonStrategy = ComparePlansWithAssertion(planDescription => {
