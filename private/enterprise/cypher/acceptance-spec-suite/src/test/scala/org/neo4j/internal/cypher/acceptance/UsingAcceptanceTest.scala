...
class UsingAcceptanceTest extends ExecutionEngineFunSuite with RunWithConfigTestSupport with CypherComparisonSupport {
  ...
  test("fail if using a variable with label not used in match") {
    ...
    // WHEN
    failWithError(Configs.All - Configs.Version2_3, "match n-->() using index n:Person(name) where n.name = 'kabam' return n",
  ...
  test("fail when equality checks are done with OR") {
    ...
    // WHEN
    failWithError(Configs.All - Configs.Version2_3, "match n-->() using index n:Person(name) where n.name = 'kabam' OR n.name = 'kaboom' return n",
