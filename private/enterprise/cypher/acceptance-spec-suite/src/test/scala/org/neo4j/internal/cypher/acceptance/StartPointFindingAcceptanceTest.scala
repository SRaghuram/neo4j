...
class StartPointFindingAcceptanceTest extends ExecutionEngineFunSuite with CypherComparisonSupport {
  ...
  test("Seek node by id with multiple values") {
    ...
    val result = executeWith(Configs.InterpretedAndSlotted, s"match (n) where id(n) IN [${n1.getId}, ${n2.getId}] return n")
  ...
  test("Seek relationship by id with multiple values") {
    ...
    val result = executeWith(Configs.InterpretedAndSlotted, s"match ()-[r]->() where id(r) IN [${rel1.getId}, ${rel2.getId}] return r")
