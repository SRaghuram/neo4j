...
class StartPointFindingAcceptanceTest extends ExecutionEngineFunSuite with CypherComparisonSupport {
  ...
  test("Can use both label scan (left) and node by id (right) when there are no indices") {
    ...
    val result = executeWith(Configs.InterpretedAndSlottedAndPipelined, s"match (n) where n:Person and ${node.getId} = id(n) return n")
    result.columnAs[Node]("n").toList should equal(List(node))
  }
  ...
  test("Can use both label scan (right) and node by id (left) when there are no indices") {
    ...
    val result = executeWith(Configs.InterpretedAndSlottedAndPipelined, s"match (n) where ${node.getId} = id(n) and n:Person return n")
    result.columnAs[Node]("n").toList should equal(List(node))
  }
  ...
  test("Seek relationship by id given on the left") {
    ...
    val result = executeWith(Configs.InterpretedAndSlottedAndPipelined, s"match ()-[r]->() where ${rel.getId} = id(r) return r")
  ...
  test("Seek relationship by id given on the right") {
    ...
    val result = executeWith(Configs.InterpretedAndSlottedAndPipelined, s"match ()-[r]->() where id(r) = ${rel.getId} return r")
    result.columnAs[Node]("r").toList should equal(List(rel))
  }
  ...
  test("Seek relationship by id with multiple values") {
    ...
    val result = executeWith(Configs.InterpretedAndSlottedAndPipelined, s"match ()-[r]->() where id(r) IN [${rel1.getId}, ${rel2.getId}] return r")
  ...
  test("Seek relationship by id with no direction") {
    ...
    val result = executeWith(Configs.InterpretedAndSlottedAndPipelined, s"match (a)-[r]-(b) where id(r) = ${r.getId} return a,r,b")
  ...
  test("Seek relationship by id and unwind") {
    ...
    val result = executeWith(Configs.InterpretedAndSlottedAndPipelined, s"PROFILE UNWIND [${r.getId}] as rId match (a)-[r]->(b) where id(r) = rId return a,r,b",
  ...
  test("Seek relationship by id with type that is not matching") {
    ...
    val result = executeWith(Configs.InterpretedAndSlottedAndPipelined, s"match ()-[r:BAR]-() where id(r) = ${r.getId} return r")
