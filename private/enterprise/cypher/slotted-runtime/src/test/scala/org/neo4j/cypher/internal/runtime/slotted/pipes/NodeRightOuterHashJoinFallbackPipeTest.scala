...
import org.neo4j.cypher.internal.runtime.slotted.{SlottedExecutionContextFactory, SlottedPipeBuilder}
...
class NodeRightOuterHashJoinFallbackPipeTest extends CypherFunSuite {
  ...
  test("should support joining on two different variables") {
    ...
    SlottedPipeBuilder.generateSlotAccessorFunctions(rightSlots)
    ...
    SlottedPipeBuilder.generateSlotAccessorFunctions(hashSlots)
    ...
    pipe.setExecutionContextFactory(SlottedExecutionContextFactory(hashSlots))
