  ...
  def getExecutionPlan(executionGraphDefinition: ExecutionGraphDefinition): ExecutionPlan = {
    ...
    executionGraphDefinition.buffers.foreach { buffer =>
      ...
      variant match {
        ...
        case RHSStreamingBufferVariant(_) =>
        ...
        case OptionalBufferVariant(_, OptionalType) =>
          bufs(id) = new VirtualNodeHack(Map("name" -> s"OptionalBuffer[$id]", "id" -> (id: Integer)),  "Buffer")
        case OptionalBufferVariant(_, AntiType) =>
    ...
    executionGraphDefinition.buffers.foreach { buffer =>
      ...
      variant match {
        ...
        case RHSStreamingBufferVariant(ArgumentStateMapId(asmId)) =>
          rels += new VirtualRelationshipHack(bufs(id), asms(asmId), Map.empty, "USES_ASM")
