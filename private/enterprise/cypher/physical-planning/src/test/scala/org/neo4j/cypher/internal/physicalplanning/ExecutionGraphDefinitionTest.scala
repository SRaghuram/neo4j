...
class ExecutionGraphDefinitionTest extends CypherFunSuite {
  ...
  test("should plan hash join") {
    new ExecutionGraphDefinitionBuilder()
      ...
      start(graph)
        ...
        .pipeline(0, Seq(classOf[AllNodesScan]))
        .leftOfJoinBuffer(id = 3, TopLevelArgument.SLOT_OFFSET, asmId = 0, planId = 1, rhsAsmId = 1, InnerVariant)
  ...
  test("should plan cartesian product") {
    new ExecutionGraphDefinitionBuilder()
      ...
      start(graph)
        .morselBuffer(4)
        .pipeline(1, Seq(classOf[NodeByLabelScan]))
        .rightOfJoinBuffer(lhsId = 5, rhsId = 6, sourceId = 7, argumentSlotOffset = 0, lhsAsmId = 0, rhsAsmId = 1, planId = 1, CartesianProductVariant)
  ...
  def operatorFuserFactory(fusablePlans: Class[_ <: LogicalPlan]*): OperatorFuserFactory[Nothing] =
    new OperatorFuserFactory[Nothing] {
      val fusable: Set[Class[_ <: LogicalPlan]] = fusablePlans.toSet
      override def newOperatorFuser(headPlanId: Id): OperatorFuser[Nothing] =
