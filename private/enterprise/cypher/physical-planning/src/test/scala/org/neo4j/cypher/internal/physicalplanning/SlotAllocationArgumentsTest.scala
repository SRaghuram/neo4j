...
class SlotAllocationArgumentsTest extends CypherFunSuite with LogicalPlanningTestSupport2 {
  ...
  test("zero size argument for single all nodes scan") {
    ...
    // when
    val arguments = SlotAllocation.allocateSlots(plan, semanticTable, I_BREAK_FOR_LEAFS).argumentSizes
  ...
  test("zero size argument for only leaf operator") {
    ...
    // when
    val arguments = SlotAllocation.allocateSlots(expand, semanticTable, I_BREAK_FOR_LEAFS).argumentSizes
  ...
  test("zero size argument for argument operator") {
    ...
    // when
    val arguments = SlotAllocation.allocateSlots(argument, semanticTable, I_BREAK_FOR_LEAFS).argumentSizes
