...
class DirectedRelationshipTypeScanOperator(val workIdentity: WorkIdentity,
                                           relOffset: Int,
                                           startOffset: Int,
                                           typ: LazyType,
                                           endOffset: Int,
  ...
  override protected def nextTasks(state: PipelinedQueryState,
    ...
    singletonIndexedSeq(new DirectedTypeScanTask(inputMorsel.nextCopy))
  }

  class DirectedTypeScanTask(inputMorsel: Morsel) extends InputLoopTask(inputMorsel) {

    override def workIdentity: WorkIdentity = DirectedRelationshipTypeScanOperator.this.workIdentity

    override def toString: String = "DirectedRelationshipTypeScanTask"
    ...
    override protected def innerLoop(outputRow: MorselFullCursor, state: PipelinedQueryState): Unit = {
      val read = state.queryContext.transactionalContext.dataRead
      while (outputRow.onValidRow() && typeIndexCursor.next()) {
