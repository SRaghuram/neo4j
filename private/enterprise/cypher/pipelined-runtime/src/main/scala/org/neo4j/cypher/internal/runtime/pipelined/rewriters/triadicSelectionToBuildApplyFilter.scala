 ...
 *   LHS
 */
case class triadicSelectionToBuildApplyFilter(effectiveCardinalities: EffectiveCardinalities,
  ...
  private val instance: Rewriter = bottomUp(Rewriter.lift {
    case triSelection @ TriadicSelection(lhs, rhs, positivePredicate, sourceId, seenId, targetId) =>
      ...
      effectiveCardinalities.copy(from = lhs.id, to = triBuild.id)
      providedOrders.copy(from = lhs.id, to = triBuild.id)

      val apply = Apply(triBuild, rhs)(idGen)
      effectiveCardinalities.copy(from = rhs.id, to = apply.id)
      ...
      effectiveCardinalities.copy(from = triSelection.id, to = triFilter.id)
