...
case class AggregationOperator(workIdentity: WorkIdentity,
  ...
  class AggregationReduceOperator(val argumentStateMapId: ArgumentStateMapId,
    ...
    override def nextTasks(state: PipelinedQueryState,
                           input: IndexedSeq[AggregatedRowMap],
                           resources: QueryResources,
                           argumentStateMaps: ArgumentStateMaps): IndexedSeq[ContinuableOperatorTaskWithAccumulators[AnyRef, AggregatedRowMap]] = {
  ...
  override def genOperate: IntermediateRepresentation = {
    ...
    block(
      ...
      declareAndAssign(typeRefOf[AggregatedRow],
        aggregatedRow,
        invoke(load(aggregatedRowMapVar), method[AggregatedRowMap, AggregatedRow, AnyValue]("get"), load[AnyValue](groupingValue))),
       ...
       * updaters.addUpdate(n-1, aggregationExpression[n-1]())
       */
      addUpdates(load[AggregatedRowUpdaters](updaters)),
