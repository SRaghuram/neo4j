...
import org.neo4j.cypher.internal.runtime.pipelined.operators.OperatorCodeGenHelperTemplates.INPUT_SINGLE_ACCUMULATOR_CONSTRUCTOR_PARAMETER
...
/**
 * Extended in Java abstract class [[CompiledStreamingOperatorWithMorsel]]
 * which is used as the base class for operator classes generated by [[CompiledStreamingOperatorWithMorselTemplate]].
 */
trait _CompiledStreamingOperatorWithMorsel extends StreamingOperator {
  /**
...
/**
 * Extended in Java abstract class [[CompiledStreamingOperatorWithMorselData]]
 * which is used as the base class for operator classes generated by [[CompiledStreamingOperatorWithMorselDataTemplate]].
 */
trait _CompiledStreamingOperatorWithMorselData extends Operator with DataInputOperatorState[AnyRef] {
...
/**
 * Extended in Java abstract class [[CompiledStreamingOperatorWithAccumulators]]
 * which is used as the base class for operator classes generated by [[CompiledStreamingOperatorWithAccumulatorsTemplate]].
 */
trait _CompiledStreamingOperatorWithAccumulators extends Operator with AccumulatorsInputOperatorState[AnyRef, MorselAccumulator[AnyRef]] {
  ...
  override def nextTasks(state: PipelinedQueryState,
                         input: IndexedSeq[MorselAccumulator[AnyRef]],
                         resources: QueryResources,
                         argumentStateMaps: ArgumentStateMaps): IndexedSeq[ContinuableOperatorTaskWithAccumulators[AnyRef, MorselAccumulator[AnyRef]]] = {
    singletonIndexedSeq(compiledNextTask(state.queryContext.transactionalContext.transaction, input, argumentStateMaps, input.head))
  }

  protected def compiledNextTask(tx: KernelTransaction,
                                 inputAccumulators: IndexedSeq[MorselAccumulator[AnyRef]],
                                 argumentStateMaps: ArgumentStateMaps,
                                 singleInputAccumulator: MorselAccumulator[AnyRef]): ContinuableOperatorTaskWithAccumulators[AnyRef, MorselAccumulator[AnyRef]]

}

object CompiledStreamingOperatorWithMorselTemplate extends CompiledStreamingOperatorTemplate[CompiledStreamingOperatorWithMorsel] {
  ...
  override protected def genExtendsClass: Option[codegen.TypeReference] = Some(inputOperatorType)
  override protected def genImplementsInterfaces: Seq[codegen.TypeReference] = Seq.empty[codegen.TypeReference]
...
case class CompiledStreamingOperatorWithAccumulatorsTemplate(accumulatorsPerTask: Int)
  extends CompiledStreamingOperatorTemplate[CompiledStreamingOperatorWithAccumulators] {
  ...
  override protected def genCompiledNextTask(taskClassHandle: ClassHandle): MethodDeclaration = {
    MethodDeclaration("compiledNextTask",
      ...
      parameters = Seq(
        ...
        param[ArgumentStateMaps]("argumentStateMaps"),
        param[MorselAccumulator[AnyRef]]("singleInputAccumulator")),
      body = newInstance(Constructor(taskClassHandle,
        Seq(TypeReference.typeReference(classOf[KernelTransaction]),
  ...
  override protected def genExtendsClass: Option[codegen.TypeReference] = Some(inputOperatorType)
  override protected def genImplementsInterfaces: Seq[codegen.TypeReference] = Seq.empty
...
/**
 * Base trait for generating compiled streaming operators
 */
trait CompiledStreamingOperatorTemplate[T <: Operator] {
  ...
  protected def genExtendsClass: Option[codegen.TypeReference] = None
  protected def genImplementsInterfaces: Seq[codegen.TypeReference] = Seq(inputOperatorType)
...
object ContinuableOperatorTaskWithMorselGenerator {
  ...
  // Choose a matching template to generate the operator class
...
trait ContinuableOperatorTaskWithSingleAccumulatorTemplate extends ContinuableOperatorTaskTemplate {
  ...
  override protected def genConstructorParameters: Seq[Parameter] = Seq(
    ...
    ARGUMENT_STATE_MAPS_CONSTRUCTOR_PARAMETER,
    INPUT_SINGLE_ACCUMULATOR_CONSTRUCTOR_PARAMETER
  )
  ...
  override protected def genImplementsInterfaces: Seq[TypeReference] = Seq(typeRefOf[CompiledTaskWithAccumulators[_, MorselAccumulator[_]]])
    ...
    ClassDeclaration[CompiledTask](
      ...
      methods = Seq(
        MethodDeclaration("initializeProfileEvents",
          ...
          body = if (doProfile) genInitializeProfileEvents else noop()
        ),
        ...
        MethodDeclaration("closeProfileEvents",
          ...
          body = if (doProfile) genCloseProfileEvents else noop()
        ),
      ...
      // NOTE: This has to be called after genOperate!
      genFields = () => staticFields ++ Seq(DATA_READ, genInputCursorField) ++ flatMap[Field](op =>
        op.genFields ++
