...
class ProduceResultOperator(val workIdentity: WorkIdentity,
  ...
  override protected def nextTasks(state: PipelinedQueryState,
    ...
    singletonIndexedSeq(new InputOTask(inputMorsel.nextCopy))
    ...
    while (output.onValidRow && served < demand) {
      ...
      while (i < expressions.length) {
        ...
        if (state.prePopulateResults) {
          ValuePopulation.populate(
            value,
            resources.expressionCursors.nodeCursor,
            resources.expressionCursors.relationshipScanCursor,
            resources.expressionCursors.propertyCursor
          )
        }
...
class ProduceResultOperatorTaskTemplate(val inner: OperatorTaskTemplate,
  ...
  override def genOperate: IntermediateRepresentation = {
    ...
    block(
      ...
      assign(SERVED, add(load(SERVED), 1L)),
