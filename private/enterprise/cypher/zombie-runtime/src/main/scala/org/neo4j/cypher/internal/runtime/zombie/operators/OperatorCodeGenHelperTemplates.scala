...
object OperatorCodeGenHelperTemplates {
  ...
  val ALLOCATE_NODE_CURSOR: IntermediateRepresentation =
    invoke(
      invoke(load("cursorPools"), method[CursorPools, CursorPool[_]]("nodeCursorPool")),
  ...
  def nodeCursor(name: String): InstanceField = field[NodeCursor](name)
  ...
  def freeNodeCursor(cursor: IntermediateRepresentation): IntermediateRepresentation =
    invokeSideEffect(
      invoke(load("cursorPools"), method[CursorPools, CursorPool[_]]("nodeCursorPool")),
      method[CursorPool[NodeCursor], Unit, Cursor]("free"), cursor)
}
