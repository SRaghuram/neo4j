...
import com.neo4j.causalclustering.helper.RobustJobSchedulerWrapper;
...
public class HazelcastClient extends SafeLifecycle implements TopologyService
{
    ...
    {
        CoreTopology newCoreTopology = hzInstance.apply( hz -> HazelcastClusterTopology.getCoreTopology( hz, config, log ) );
        coreTopology = newCoreTopology;
        localCoreTopology = newCoreTopology.filterTopologyByDb( dbName );

        ReadReplicaTopology newReadReplicaTopology = hzInstance.apply( hz -> HazelcastClusterTopology.getReadReplicaTopology( hz, log ) );
        readReplicaTopology = newReadReplicaTopology;
        localReadReplicaTopology = newReadReplicaTopology.filterTopologyByDb( dbName );

        catchupAddressMap = HazelcastClusterTopology.extractCatchupAddressesMap( localCoreServers(), localReadReplicas() );
    }
    ...
    private void keepReadReplicaAlive() throws HazelcastInstanceNotActiveException
    {
        hzInstance.perform( hazelcastInstance ->
        {
            ...
            HazelcastClusterTopology.refreshGroups( hazelcastInstance, hzId, groups );
