...
public class IdFilesSanitationModule extends LifecycleAdapter
{
    ...
    public void start() throws Throwable
    {
        ...
        {
            Optional<DatabaseContext> dbContext = databaseManager.getDatabaseContext( databaseName );
            dbContext.ifPresent( db -> deleteIdFiles( databaseName, db.getDatabase().getDatabaseLayout() ) );
        }
    }

    private void deleteIdFiles( String databaseName, DatabaseLayout databaseLayout )
    {
        if ( deleteIdFiles( databaseLayout ) )
        {
