...
public class CoreBootstrapper
{
            ...
            FileSystemAbstraction fs, Config activeDatabaseConfig, LogProvider logProvider, PageCache pageCache )
    {
        ...
        this.activeDatabaseParams = filterActiveDatabaseConfig( activeDatabaseConfig );
        this.systemDatabaseParams = initialSystemDatabaseParams();
    ...
    private Map<String,String> filterActiveDatabaseConfig( Config activeDatabaseConfig )
    {
        Map<String,String> params = new HashMap<>();

        /* We want to only inherit things that will affect the storage as necessary during bootstrap of the database. */
        params.put( GraphDatabaseSettings.active_database.name(), activeDatabaseConfig.get( active_database ) );
    ...
    private Map<String,String> initialSystemDatabaseParams()
    {
        Map<String,String> params = new HashMap<>();

        /* Makes it so that tx logs end up in the the database folder, see {@link LogFilesBuilder). */
    ...
    void bootstrapDatabase( LocalDatabase database, Map<String,String> params, Config config, DatabaseInitializer databaseInitializer )
            throws IOException, RecoveryRequiredException
    {
        DatabaseLayout databaseLayout = database.databaseLayout();

        ensureRecoveredOrThrow( databaseLayout, config );

        if ( NeoStores.isStorePresent( pageCache, databaseLayout ) )
        {
            recoverMissingFiles( databaseLayout, config );
            return;
        }
