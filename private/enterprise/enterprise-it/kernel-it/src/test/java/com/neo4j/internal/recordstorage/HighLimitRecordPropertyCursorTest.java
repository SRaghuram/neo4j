...
class HighLimitRecordPropertyCursorTest extends RecordPropertyCursorTest
{
    ...
    void mustThrowOnPropertyRecordDecodingErrors() throws Exception
    {
        ...
        try ( PagedFile pagedFile = pageCache.map( propertyStore.getStorageFile(), 0, DEFAULT_DATABASE_NAME, Sets.immutable.of( ANY_PAGE_SIZE ) );
    ...
    void mustThrowOnArrayDecodingErrors() throws Exception
    {
        ...
        try ( PagedFile pagedFile = pageCache.map( arrayStore.getStorageFile(), 0, DEFAULT_DATABASE_NAME, Sets.immutable.of( ANY_PAGE_SIZE ) );
              PageCursor cursor = pagedFile.io( 0, PagedFile.PF_SHARED_WRITE_LOCK, NULL ) )
        {
    ...
    void mustThrowOnStringDecodingErrors() throws Exception
    {
        ...
        try ( PagedFile pagedFile = pageCache.map( stringStore.getStorageFile(), 0, DEFAULT_DATABASE_NAME, Sets.immutable.of( ANY_PAGE_SIZE ) );
              PageCursor cursor = pagedFile.io( 0, PagedFile.PF_SHARED_WRITE_LOCK, NULL ) )
        {
