...
class EnterpriseRuntimeUnsupportedNotificationTest extends ExecutionEngineFunSuite with EnterpriseGraphDatabaseTestSupport {
  ...
  test("Should give specific error message why pipelined does not support a query") {
    val result = execute("CYPHER runtime=pipelined EXPLAIN MATCH (n) RETURN (n)-->()")
    result.notifications should contain(RUNTIME_UNSUPPORTED.notification(InputPosition.empty,
      NotificationDetail.Factory.message("Runtime unsupported", "Pipelined does not yet support the plans including `RollUpApply`, use another runtime.")))
  }
  ...
  test("can also be configured to fail hard") {
    ...
    val exception = intercept[RuntimeUnsupportedException](execute("CYPHER runtime=pipelined EXPLAIN MATCH (n) RETURN (n)-->()"))
    exception.getMessage should be("Pipelined does not yet support the plans including `RollUpApply`, use another runtime.")
  }
}
