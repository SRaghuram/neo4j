...
import com.neo4j.test.extension.dbms.CommercialDbmsExtensionEnforceAnnotations;
import com.neo4j.test.extension.dbms.CommercialDbmsExtensionMixImpermanent;
...
class CommercialDbmsExtensionTest
{
    @Test
    void enforceAnnotation()
    {
        Events testEvents = EngineTestKit.engine( ENGINE_ID )
                .selectors( selectClass( CommercialDbmsExtensionEnforceAnnotations.class ) ).execute()
                .all();
    ...
    {
        Events testEvents = EngineTestKit.engine( ENGINE_ID )
                .selectors( selectClass( CommercialDbmsExtensionMixImpermanent.NestedTest.class ) ).execute()
                .all();
