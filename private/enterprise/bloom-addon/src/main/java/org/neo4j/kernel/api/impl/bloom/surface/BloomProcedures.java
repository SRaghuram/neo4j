 ...
 */
package org.neo4j.kernel.api.impl.bloom.surface;
...
import org.neo4j.kernel.api.impl.bloom.FulltextProvider;
import org.neo4j.kernel.api.impl.bloom.ReadOnlyFulltext;
import org.neo4j.kernel.api.impl.bloom.ScoreEntityIterator;
import org.neo4j.kernel.api.impl.bloom.ScoreEntityIterator.ScoreEntry;
import org.neo4j.procedure.Context;
import org.neo4j.procedure.Description;
import org.neo4j.procedure.Name;
import org.neo4j.procedure.Procedure;

import static org.neo4j.kernel.api.impl.bloom.FulltextIndexType.NODES;
import static org.neo4j.kernel.api.impl.bloom.FulltextIndexType.RELATIONSHIPS;
...
public class BloomProcedures
{
    ...
    {
        return provider.getProperties( BloomKernelExtensionFactory.BLOOM_NODES, NODES ).stream().map( PropertyOutput::new );
    }
    ...
    {
        return provider.getProperties( BloomKernelExtensionFactory.BLOOM_RELATIONSHIPS, RELATIONSHIPS ).stream().map( PropertyOutput::new );
    }
    ...
    {
        provider.changeIndexedProperties( BloomKernelExtensionFactory.BLOOM_NODES, NODES, propertyKeys );
    }
    ...
    {
        provider.changeIndexedProperties( BloomKernelExtensionFactory.BLOOM_RELATIONSHIPS, RELATIONSHIPS, propertyKeys );
    }
    ...
    {
        StatusOutput nodeIndexState = new StatusOutput( BloomKernelExtensionFactory.BLOOM_NODES, provider.getState( BloomKernelExtensionFactory.BLOOM_NODES, NODES ) );
        StatusOutput relationshipIndexState = new StatusOutput( BloomKernelExtensionFactory.BLOOM_RELATIONSHIPS, provider.getState( BloomKernelExtensionFactory.BLOOM_RELATIONSHIPS, RELATIONSHIPS ) );
    ...
    {
        try ( ReadOnlyFulltext indexReader = provider.getReader( BloomKernelExtensionFactory.BLOOM_NODES, NODES ) )
        {
    ...
    public Stream<EntityOutput> bloomFulltextRelationships(
            ...
            @Name( value = "matchAll", defaultValue = "false" ) boolean matchAll ) throws Exception
    {
        try ( ReadOnlyFulltext indexReader = provider.getReader( BloomKernelExtensionFactory.BLOOM_RELATIONSHIPS, RELATIONSHIPS ) )
        {
            return queryAsStream( terms, indexReader, fuzzy, matchAll );
        }
    }
