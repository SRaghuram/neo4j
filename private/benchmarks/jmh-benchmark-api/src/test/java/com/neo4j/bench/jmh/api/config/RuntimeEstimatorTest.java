...
import static com.neo4j.bench.jmh.api.config.RuntimeEstimator.DEFAULT_STORE_GENERATION_DURATION;
import static com.neo4j.bench.jmh.api.config.RuntimeEstimator.MISCELLANEOUS_OVERHEAD;
import static org.hamcrest.MatcherAssert.assertThat;
import static org.hamcrest.Matchers.equalTo;

public class RuntimeEstimatorTest extends AnnotationsFixture
{
    @Test
    public void shouldEstimateExecutionCountForOneBenchmark1InSingleThreadedMode()
    {
        Validation validation = new Validation();
        List<BenchmarkDescription> benchmarks = newArrayList( of( ValidEnabledBenchmark1.class, validation, getAnnotations() ) );
    ...
    {
        Validation validation = new Validation();
        List<BenchmarkDescription> benchmarks = newArrayList( of( ValidEnabledBenchmark2.class, validation, getAnnotations() ) );
        int[] threadCounts = new int[]{1};
    ...
    public void shouldEstimateExecutionCountForTwoBenchmarksInSingleThreadedMode()
    {
        ...
        List<BenchmarkDescription> benchmarks = newArrayList(
                of( ValidEnabledBenchmark1.class, validation, getAnnotations() ), // 6 && not thread safe --> 6
                of( ValidEnabledBenchmark2.class, validation, getAnnotations() )  // 1 && thread safe     --> 1
        );
        int[] threadCounts = new int[]{1};
        String[] jmhArgs = "-i 2 -wi 2 -r 5 -w 5 -f 3".split( " " );
    ...
    public void shouldEstimateExecutionCountForTwoBenchmarksInMultiThreadedMode()
    {
        Validation validation = new Validation();
        List<BenchmarkDescription> benchmarks = newArrayList(
                of( ValidEnabledBenchmark1.class, validation, getAnnotations() ), // 6 && not thread safe --> 0
                of( ValidEnabledBenchmark2.class, validation, getAnnotations() )  // 1 && thread safe     --> 1
        );
    ...
    public void shouldEstimateExecutionCountForTwoBenchmarksInMixedThreadedMode()
    {
        Validation validation = new Validation();
        List<BenchmarkDescription> benchmarks = newArrayList(
                of( ValidEnabledBenchmark1.class, validation, getAnnotations() ), // 6 && not thread safe --> 6 <-- 6 * 1
                of( ValidEnabledBenchmark2.class, validation, getAnnotations() )  // 1 && thread safe     --> 5 <-- 1 * 5 (thread modes)
        );
