...
public class LongQuery5EmbeddedCore_1 extends Neo4jQuery5<Neo4jConnectionState>
{
            ...
            throws DbException
    {
        ...
        // Forum [Friends]
        Map<Node,Set<Node>> friendsInForumsWithPosts = new HashMap<>();
        for ( Node friend : friends )
        {
            for ( Relationship hasMember : friend.getRelationships( Direction.INCOMING, Rels.HAS_MEMBER_WITH_POSTS ) )
            {
                long joinDate = (long) hasMember.getProperty( HasMember.JOIN_DATE );
                if ( joinDate > minDate )
                {
                    Node forum = hasMember.getStartNode();
                    Set<Node> friendsInForum = friendsInForumsWithPosts.get( forum );
                    if ( null == friendsInForum )
                    {
                        friendsInForum = new HashSet<>();
                        friendsInForumsWithPosts.put( forum, friendsInForum );
                    }
        ...
        for ( Node forum : friendsInForumsWithPosts.keySet() )
        {
        ...
        RelationshipType[] hasMemberAtTime =
                connection.timeStampedRelationshipTypesCache().hasMemberForDatesAfter(
                        dateUtil.formatToEncodedDateAtResolution( minDate )
                );

        if ( preResults.size() < operation.limit() )
        {
            Set<Node> forumsWithoutPosts = new HashSet<>();
            for ( Node friend : friends )
            {
                for ( Relationship hasMember : friend.getRelationships( Direction.INCOMING, hasMemberAtTime ) )
                {
                    long joinDate = (long) hasMember.getProperty( HasMember.JOIN_DATE );
                    if ( joinDate > minDate )
                    {
                        Node forum = hasMember.getStartNode();
                        if ( !friendsInForumsWithPosts.containsKey( forum ) )
                        {
                            forumsWithoutPosts.add( forum );
                        }
                    }
                }
            }

            for ( Node forum : forumsWithoutPosts )
            {
                preResults.add( new LdbcQuery5PreResult( forum, 0 ) );
            }
        }
