...
public class Neo4jArchive
{
    ...
    public static List<String> extractJvmArgs( File neo4jPackage )
    {
        Neo4jArchive neo4jArchive = new Neo4jArchive( neo4jPackage );
        if ( !neo4jArchive.hasDefaultJvmArgs() )
        {
            throw new RuntimeException( format( "No JVM args found in package: %s", neo4jPackage.getAbsolutePath() ) );
        }
        return neo4jArchive.defaultJvmArgs();
    }

    Neo4jArchive( File archive )
    {
