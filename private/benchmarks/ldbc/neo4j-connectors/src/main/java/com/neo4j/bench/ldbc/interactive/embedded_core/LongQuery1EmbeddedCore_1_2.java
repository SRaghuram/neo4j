...
public class LongQuery1EmbeddedCore_1_2 extends Neo4jQuery1<Neo4jConnectionState>
{
            ...
            throws DbException
    {
        ...
        Node startPerson = Operators.findNode( connection.db(), Nodes.Person, Person.ID, operation.personId() );

        Set<Node> personsWithFirstName = Sets.newHashSet(
                connection.db().findNodes( Nodes.Person, Person.FIRST_NAME, operation.firstName() )
        );
    ...
    private static class Query1ResultProjectionFunction
    {
                ...
                QueryDateUtil dateUtil )
        {
            ...
            Map<String,Object> personProperties = person.getProperties(
                    Person.BIRTHDAY,
            ...
            long birthday = dateUtil.formatToUtc( (long) personProperties.get( Person.BIRTHDAY ) );
            long creationDate = dateUtil.formatToUtc( (long) personProperties.get( Person.CREATION_DATE ) );
            String gender = (String) personProperties.get( Person.GENDER );
            List<String> languages = Lists.newArrayList( (String[]) personProperties.get( Person.LANGUAGES ) );
            String browser = (String) personProperties.get( Person.BROWSER_USED );
            String ip = (String) personProperties.get( Person.LOCATION_IP );
            List<String> emails = Lists.newArrayList( (String[]) personProperties.get( Person.EMAIL_ADDRESSES ) );
            ...
            for ( Relationship worksAt : person.getRelationships( Direction.OUTGOING, worksAtRelationshipTypes ) )
            {
