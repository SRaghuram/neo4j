...
for i in "${dbs[@]}"; do
    ...
    aws s3 cp s3://storage.benchmarking.neo4j.today/datasets/ldbc/db/"${old_tar}" . --no-progress
    ...
    aws s3 cp "${new_tar}" s3://storage.benchmarking.neo4j.today/datasets/ldbc/db/"${new_tar}" --no-progress
    rm -rf "${old_db_path}"
    rm -rf "${new_db_path}"
