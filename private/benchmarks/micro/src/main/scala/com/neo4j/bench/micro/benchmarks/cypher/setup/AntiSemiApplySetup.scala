...
class AntiSemiApplySetup extends AbstractCypherBenchmark {
  @ParamValues(
  ...
  override def description = "AntiSemiApply setup"

  override def setup(planContext: PlanContext): TestSetup = {
    val initialPlanBuilder = new BenchmarkSetupPlanBuilder()
      .produceResults("n")

    val planBuilderWithSemiApplies = (1 to depth).foldLeft(initialPlanBuilder) { (accumulatingBuilder, _) =>
      accumulatingBuilder
        .antiSemiApply()
        .|.allNodeScan("m", "n")
    }
  ...
  def executePlan(threadState: AntiSemiApplySetupThreadState, bh: Blackhole): Int = {
...
@State(Scope.Thread)
class AntiSemiApplySetupThreadState {
  var tx: InternalTransaction = _
  var executablePlan: ExecutablePlan = _

  @Setup
  def setUp(benchmarkState: AntiSemiApplySetup): Unit = {
...
object AntiSemiApplySetup {
  def main(args: Array[String]): Unit = {
    Main.run(classOf[AntiSemiApplySetup])
  }
}