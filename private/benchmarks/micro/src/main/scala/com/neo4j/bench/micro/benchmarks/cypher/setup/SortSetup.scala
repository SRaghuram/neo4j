...
class SortSetup extends AbstractCypherBenchmark {
  @ParamValues(
  ...
  override def description = "Sort setup"

  override def setup(planContext: PlanContext): TestSetup = {
    val initialPlanBuilder = new BenchmarkSetupPlanBuilder()
      .produceResults("n")

    val planBuilderWithSorts = (1 to depth).foldLeft(initialPlanBuilder) { (accumulatingBuilder, number) =>
      accumulatingBuilder
        .sort(Seq(Ascending(s"prop$number")))
        .projection(s"n.prop$number AS `prop$number`")
    }

    val completePlanBuilder = planBuilderWithSorts.allNodeScan("n")
  ...
  def executePlan(threadState: SortSetupThreadState, bh: Blackhole): Int = {
...
@State(Scope.Thread)
class SortSetupThreadState {
  var tx: InternalTransaction = _
  var executablePlan: ExecutablePlan = _

  @Setup
  def setUp(benchmarkState: SortSetup): Unit = {
