...
class CrossApply extends AbstractCypherBenchmark {
  @ParamValues(
  ...
  override def description = "Cross Apply"
  ...
  override def getLogicalPlanAndSemanticTable(planContext: PlanContext): (plans.LogicalPlan, SemanticTable, List[String]) = {
    ...
    val allNodesScanB = plans.AllNodesScan(b, Set(a))(IdGen)
    val crossApply = plans.CrossApply(allNodesScanA, allNodesScanB)(IdGen)
    val resultColumns = List(a, b)
    val produceResults = plans.ProduceResult(crossApply, columns = resultColumns)(IdGen)
  ...
  def executePlan(threadState: CrossApplyThreadState, bh: Blackhole): Long = {
...
@State(Scope.Thread)
class CrossApplyThreadState {
  var tx: InternalTransaction = _
  var executablePlan: ExecutablePlan = _

  @Setup
  def setUp(benchmarkState: CrossApply): Unit = {
