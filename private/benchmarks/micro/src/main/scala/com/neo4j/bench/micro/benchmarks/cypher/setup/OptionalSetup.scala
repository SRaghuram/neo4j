...
class OptionalSetup extends AbstractCypherBenchmark {
  @ParamValues(
  ...
  override def description = "Optional setup"

  override def setup(planContext: PlanContext): TestSetup = {
    val initialPlanBuilder = new BenchmarkSetupPlanBuilder()
      .produceResults("n")

    val planBuilderWithOptionals = (1 to depth).foldLeft(initialPlanBuilder) { (accumulatingBuilder, number) =>
      accumulatingBuilder.optional("n")
    }

    val completePlanBuilder = planBuilderWithOptionals.allNodeScan("n")
  ...
  def executePlan(threadState: OptionalSetupThreadState, bh: Blackhole): Int = {
...
@State(Scope.Thread)
class OptionalSetupThreadState {
  var tx: InternalTransaction = _
  var executablePlan: ExecutablePlan = _

  @Setup
  def setUp(benchmarkState: OptionalSetup): Unit = {
...
object OptionalSetup {
  def main(args: Array[String]): Unit = {
    Main.run(classOf[OptionalSetup])
  }
}