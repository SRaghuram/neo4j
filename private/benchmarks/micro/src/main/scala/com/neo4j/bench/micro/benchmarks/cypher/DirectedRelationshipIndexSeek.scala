...
class DirectedRelationshipIndexSeek extends AbstractCypherBenchmark {
  @ParamValues(
  ...
  def executePlan(threadState: DirectedRelationshipIndexSeekThreadState, bh: Blackhole): Long = {
...
object DirectedRelationshipIndexSeek {
  def main(args: Array[String]): Unit = {
    Main.run(classOf[DirectedRelationshipIndexSeek], args:_*)
  }
}

@State(Scope.Thread)
class DirectedRelationshipIndexSeekThreadState {
  var tx: InternalTransaction = _
  var executablePlan: ExecutablePlan = _

  @Setup
  def setUp(benchmarkState: DirectedRelationshipIndexSeek): Unit = {
