...
class ConditionalApplySetup extends AbstractCypherBenchmark {
  @ParamValues(
  ...
  override def description = "ConditionalApply setup"

  override def setup(planContext: PlanContext): TestSetup = {
    val initialPlanBuilder = new BenchmarkSetupPlanBuilder()
      .produceResults("n")

    val planBuilderWithConditionalApplies = (1 to depth).foldLeft(initialPlanBuilder) { (accumulatingBuilder, number) =>
      accumulatingBuilder
        .conditionalApply("n")
        .|.allNodeScan(s"m$number", "n")
    }

    val completePlanBuilder = planBuilderWithConditionalApplies.allNodeScan("n")
  ...
  def executePlan(threadState: ConditionalApplySetupThreadState, bh: Blackhole): Int = {
...
@State(Scope.Thread)
class ConditionalApplySetupThreadState {
  var tx: InternalTransaction = _
  var executablePlan: ExecutablePlan = _

  @Setup
  def setUp(benchmarkState: ConditionalApplySetup): Unit = {
...
object ConditionalApplySetup {
  def main(args: Array[String]): Unit = {
    Main.run(classOf[ConditionalApplySetup])
  }
}