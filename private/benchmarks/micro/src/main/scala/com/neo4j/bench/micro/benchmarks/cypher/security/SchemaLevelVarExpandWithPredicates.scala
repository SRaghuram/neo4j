...
class SchemaLevelVarExpandWithPredicates extends AbstractSecurityBenchmark {
  @ParamValues(
    allowed = Array(Interpreted.NAME, Slotted.NAME, Morsel.NAME, Parallel.NAME),
    base = Array(Slotted.NAME, Morsel.NAME))
  @Param(Array[String]())
  var SchemaLevelVarExpandWithPredicates_runtime: String = _

  @ParamValues(
    allowed = Array("10"),
    base = Array("10"))
  @Param(Array[String]())
  var SchemaLevelVarExpandWithPredicates_degree: Int = _

  @ParamValues(
    allowed = Array("1", "2"),
    base = Array("2"))
  @Param(Array[String]())
  var SchemaLevelVarExpandWithPredicates_minDepth: Int = _

  @ParamValues(
    allowed = Array("1", "2", "3"),
    base = Array("3"))
  @Param(Array[String]())
  var SchemaLevelVarExpandWithPredicates_length: Int = _
  ...
  @Param(Array[String]())
  var SchemaLevelVarExpandWithPredicates_user: String = _
  ...
  override protected def getConfig: DataGeneratorConfig = {
    new DataGeneratorConfigBuilder()
      ...
      .withOutRelationships(new RelationshipDefinition(relType, SchemaLevelVarExpandWithPredicates_degree))
  ...
  override def getLogicalPlanAndSemanticTable(planContext: PlanContext): (plans.LogicalPlan, SemanticTable, List[String]) = {
    ...
    val expand = plans.VarExpand(
      ...
      relName,
      VarPatternLength(
        min = SchemaLevelVarExpandWithPredicates_minDepth,
        max = Some(SchemaLevelVarExpandWithPredicates_minDepth + SchemaLevelVarExpandWithPredicates_length)),
      ExpandAll,
...
class VarExpandVarExpandWithPredicatesThreadState {
  ...
  @Setup
  def setUp(benchmarkState: SchemaLevelVarExpandWithPredicates): Unit = {
    executablePlan = benchmarkState.buildPlan(from(benchmarkState.SchemaLevelVarExpandWithPredicates_runtime))
    tx = benchmarkState.beginInternalTransaction(benchmarkState.users(benchmarkState.SchemaLevelVarExpandWithPredicates_user))
  }
...
object SchemaLevelVarExpandWithPredicates {
  def main(args: Array[String]): Unit = {
    Main.run(classOf[SchemaLevelVarExpandWithPredicates], args:_*)
  }
}
