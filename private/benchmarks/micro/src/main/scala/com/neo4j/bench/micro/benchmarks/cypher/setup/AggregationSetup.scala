...
class AggregationSetup extends AbstractCypherBenchmark {
  @ParamValues(
  ...
  override def description = "Aggregation setup"

  override def setup(planContext: PlanContext): TestSetup = {
    val initialPlanBuilder = new BenchmarkSetupPlanBuilder()
      .produceResults("count1")

    val planBuilderWithAggregations = (1 until depth).foldLeft(initialPlanBuilder) { (accumulatingBuilder, number) =>
      accumulatingBuilder.aggregation(Seq(), Seq(s"count(count${number+1}) AS count$number"))
    }

    val completePlanBuilder = planBuilderWithAggregations
      .aggregation(Seq(), Seq(s"count(n) as count$depth"))
      .allNodeScan("n")
  ...
  def executePlan(threadState: AggregationSetupThreadState, bh: Blackhole): Int = {
...
@State(Scope.Thread)
class AggregationSetupThreadState {
  var tx: InternalTransaction = _
  var executablePlan: ExecutablePlan = _

  @Setup
  def setUp(benchmarkState: AggregationSetup): Unit = {
...
object AggregationSetup {
  def main(args: Array[String]): Unit = {
    Main.run(classOf[AggregationSetup])
  }
}