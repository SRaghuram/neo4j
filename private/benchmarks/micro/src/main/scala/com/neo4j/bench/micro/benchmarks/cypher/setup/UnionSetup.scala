...
class UnionSetup extends AbstractCypherBenchmark {
  @ParamValues(
  ...
  override def description = "Union setup"

  override def setup(planContext: PlanContext): TestSetup = {
    val initialPlanBuilder = new BenchmarkSetupPlanBuilder()
      .produceResults("n")

    val planBuilderWithUnions = (1 to depth).foldLeft(initialPlanBuilder) { (accumulatingBuilder, number) =>
      accumulatingBuilder
        .union()
        .|.allNodeScan("n")
    }

    val completePlanBuilder = planBuilderWithUnions.allNodeScan("n")
  ...
  def executePlan(threadState: UnionSetupThreadState, bh: Blackhole): Int = {
...
@State(Scope.Thread)
class UnionSetupThreadState {
  var tx: InternalTransaction = _
  var executablePlan: ExecutablePlan = _

  @Setup
  def setUp(benchmarkState: UnionSetup): Unit = {
