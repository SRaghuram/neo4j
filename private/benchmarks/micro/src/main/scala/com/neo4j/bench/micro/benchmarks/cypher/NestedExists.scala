...
class NestedExists extends AbstractCypherBenchmark {
  @ParamValues(
  ...
  override def setup(planContext: PlanContext): TestSetup = {
    ...
    val lhsScan = plans.AllNodesScan("ignore", Set.empty)(IdGen)
    val projection = plans.Projection(lhsScan, Map("exists" -> astNestedExists(filter)))(IdGen)
  ...
  def executePlan(threadState: NestedExistsThreadState, bh: Blackhole): Long = {
...
object NestedExists {
  def main(args: Array[String]): Unit = {
    Main.run(classOf[NestedExists], args:_*)
  }
}

@State(Scope.Thread)
class NestedExistsThreadState {
  var tx: InternalTransaction = _
  var executablePlan: ExecutablePlan = _

  @Setup
  def setUp(benchmarkState: NestedExists): Unit = {
