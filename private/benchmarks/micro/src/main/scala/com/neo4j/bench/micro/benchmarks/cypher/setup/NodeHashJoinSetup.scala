...
class NodeHashJoinSetup extends AbstractCypherBenchmark {
  @ParamValues(
  ...
  override def description = "NodeHashJoin setup"

  override def setup(planContext: PlanContext): TestSetup = {
    val initialPlanBuilder = new BenchmarkSetupPlanBuilder()
      .produceResults("n")

    val planBuilderWithJoins = (1 to depth).foldLeft(initialPlanBuilder) { (accumulatingBuilder, _) =>
      accumulatingBuilder
        .nodeHashJoin("n")
        .|.projection("m as n")
        .|.allNodeScan("m")
    ...
    val completePlanBuilder = planBuilderWithJoins.allNodeScan("n")
  ...
  def executePlan(threadState: NodeHashJoinSetupThreadState, bh: Blackhole): Int = {
...
@State(Scope.Thread)
class NodeHashJoinSetupThreadState {
  var tx: InternalTransaction = _
  var executablePlan: ExecutablePlan = _

  @Setup
  def setUp(benchmarkState: NodeHashJoinSetup): Unit = {
...
object NodeHashJoinSetup {
  def main(args: Array[String]): Unit = {
    Main.run(classOf[NodeHashJoinSetup])
  }
}