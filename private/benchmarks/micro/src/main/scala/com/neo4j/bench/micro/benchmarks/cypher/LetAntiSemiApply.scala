...
class LetAntiSemiApply extends AbstractCypherBenchmark {
  @ParamValues(
  ...
  override def description = "Let Anti Semi Apply"
  ...
  override def getLogicalPlanAndSemanticTable(planContext: PlanContext): (plans.LogicalPlan, SemanticTable, List[String]) = {
    ...
    val letAntiSemiApply = plans.LetAntiSemiApply(lhsAllNodesScan, indexSeek, idName)(IdGen)
    val resultColumns = List(lhs, idName)
    val produceResults = plans.ProduceResult(letAntiSemiApply, columns = resultColumns)(IdGen)
...
@State(Scope.Thread)
class LetAntiSemiApplyThreadState {
  var tx: InternalTransaction = _
  var executablePlan: ExecutablePlan = _

  @Setup
  def setUp(benchmarkState: LetAntiSemiApply): Unit = {
...
object LetAntiSemiApply {
  def main(args: Array[String]): Unit = {
    Main.run(classOf[LetAntiSemiApply])
  }
}
