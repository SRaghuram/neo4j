...
class SemiApplySetup extends AbstractCypherBenchmark {
  @ParamValues(
  ...
  override def description = "SemiApply setup"

  override def setup(planContext: PlanContext): TestSetup = {
    val initialPlanBuilder = new BenchmarkSetupPlanBuilder()
      .produceResults("n")

    val planBuilderWithSemiApplies = (1 to depth).foldLeft(initialPlanBuilder) { (accumulatingBuilder, number) =>
      accumulatingBuilder
        .semiApply()
        .|.allNodeScan(s"m$number", "n")
    }

    val completePlanBuilder = planBuilderWithSemiApplies.allNodeScan("n")
  ...
  def executePlan(threadState: SemiApplySetupThreadState, bh: Blackhole): Int = {
...
@State(Scope.Thread)
class SemiApplySetupThreadState {
  var tx: InternalTransaction = _
  var executablePlan: ExecutablePlan = _

  @Setup
  def setUp(benchmarkState: SemiApplySetup): Unit = {
...
object SemiApplySetup {
  def main(args: Array[String]): Unit = {
    Main.run(classOf[SemiApplySetup])
  }
}