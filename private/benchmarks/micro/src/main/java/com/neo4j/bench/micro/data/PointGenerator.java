...
public abstract class PointGenerator
{
    ...
    public static class ClusterGridDefinition
    {
                ...
                double extentY )
        {
            if ( clusterCountX <= 0 )
            {
                throw new Kaboom( "Must be at least 1 cluster in x-dim, but was: " + clusterCountX );
            }
            if ( clusterCountY <= 0 )
            {
                throw new Kaboom( "Must be at least 1 cluster in y-dim, but was: " + clusterCountY );
            }
            if ( clusterSizeX > extentX )
            {
                throw new Kaboom( format(
        ...
        public ValueGeneratorFun<Point> create()
        {
            ...
            return new ValueGeneratorFun<>()
            {
                ...
                private double nextBoundedGaussian( SplittableRandom rng )
                {
                    ...
                    throw new Kaboom( "Took too long to find Gaussian value in range [-1,1]" );
                }
            };
        }
