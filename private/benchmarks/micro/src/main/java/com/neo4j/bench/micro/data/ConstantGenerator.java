...
public class ConstantGenerator
{
    ...
    public static class ConstantGeneratorFactory implements ValueGeneratorFactory
    {
        ...
        private static String asString( String type, Object maybeString )
        {
            ...
            {
                throw new Kaboom( format( "Expected '%s' to be a string but was: %s", maybeString, maybeStringClass.getName() ) );
            }
            String string = (String) maybeString;
            List<String> validTypes = Arrays.asList( STR_INL, STR_SML, STR_BIG );
            if ( !validTypes.contains( type ) )
            {
                throw new Kaboom( format( "Expected '%s' to be a valid string type %s but was: %s", string, validTypes, type ) );
            }
            if ( type.equals( STR_BIG ) )
            {
                ...
                {
                    throw new Kaboom( format( "Expected '%s':%s to have length >= %s but was: %s", string, type, BIG_STRING_LENGTH, string.length() ) );
                }
            }
            else
            {
                ...
                {
                    throw new Kaboom( format( "Expected '%s':%s to have length <= %s but was: %s", string, type, SMALL_STRING_LENGTH, string.length() ) );
                }
            }
            return string;
        }
        ...
        private static Number asNumber( Object maybeNumber )
        {
            ...
            {
                throw new Kaboom( format( "Expected '%s' to be a number but was: %s", maybeNumber, maybeNumber.getClass().getName() ) );
            }
