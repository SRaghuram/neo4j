...
class CausalClusterStatusEndpointIT
{
    ...
    void statusEndpointIsReachableAndReadable() throws Exception
    {
        ...
        for ( ServerControls core : CLUSTER.getCoreControls() )
        {
            ...
            assertEventually( serverStatusEndpoint( core ), memberIdFieldIs( not( isEmptyOrNullString() ) ), 1, TimeUnit.MINUTES );
            assertEventually( serverStatusEndpoint( core ), healthFieldIs( equalTo( true ) ), 1, TimeUnit.MINUTES );
            assertEventually( serverStatusEndpoint( core ), leaderFieldIs( not( isEmptyOrNullString() ) ), 1, TimeUnit.MINUTES );
        ...
        for ( ServerControls replica : CLUSTER.getReplicaControls() )
        {
            writeSomeData( CLUSTER );
            ...
            assertEventually( serverStatusEndpoint( replica ), memberIdFieldIs( not( isEmptyOrNullString() ) ), 1, TimeUnit.MINUTES );
            assertEventually( serverStatusEndpoint( replica ), healthFieldIs( equalTo( true ) ), 1, TimeUnit.MINUTES );
            assertEventually( serverStatusEndpoint( replica ), leaderFieldIs( not( isEmptyOrNullString() ) ), 1, TimeUnit.MINUTES );
    ...
    {
        ServerControls first = CLUSTER.getCoreControls().get( 0 );
        try
        {
            List<ServerControls> switchedOff =
                    CLUSTER.getCoreControls().stream().filter( e -> !first.equals( e ) ).peek( ServerControls::close ).collect( Collectors.toList() );
