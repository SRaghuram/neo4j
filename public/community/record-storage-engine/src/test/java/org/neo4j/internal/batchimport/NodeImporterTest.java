...
class NodeImporterTest
{
    ...
    void shouldHandleLargeAmountsOfLabels() throws IOException
    {
                      ...
                      INSTANCE ) )
        {
            ...
            long nodeId = 0;
            try ( NodeImporter importer = new NodeImporter( stores, idMapper, new DataImporterMonitor(), NULL, INSTANCE ) )
            {
                importer.id( nodeId );
    ...
    void tracePageCacheAccessOnNodeImport() throws IOException
    {
                        ...
                        INSTANCE ) )
        {
            ...
            try ( NodeImporter importer = new NodeImporter( stores, IdMappers.actual(), new DataImporterMonitor(), cacheTracer, INSTANCE ) )
            {
                importer.id( nodeId );
                String[] labels = new String[numberOfLabels];
                for ( int i = 0; i < labels.length; i++ )
