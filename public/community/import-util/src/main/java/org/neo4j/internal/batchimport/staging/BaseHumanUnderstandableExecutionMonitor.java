 ...
 */
public abstract class BaseHumanUnderstandableExecutionMonitor implements ExecutionMonitor{
    ...
    protected static final String ESTIMATED_REQUIRED_MEMORY_USAGE = "Estimated required memory usage";
    protected static final String ESTIMATED_DISK_SPACE_USAGE = "Estimated disk space usage";
    protected static final String ESTIMATED_NUMBER_OF_RELATIONSHIP_PROPERTIES = "Estimated number of relationship properties";
    protected static final String ESTIMATED_NUMBER_OF_RELATIONSHIPS = "Estimated number of relationships";
    protected static final String ESTIMATED_NUMBER_OF_NODE_PROPERTIES = "Estimated number of node properties";
    protected static final String ESTIMATED_NUMBER_OF_NODES = "Estimated number of nodes";
    protected static final int DOT_GROUP_SIZE = 10;
    protected static final int DOT_GROUPS_PER_LINE = 5;
    protected static final int PERCENTAGES_PER_LINE = 5;

    protected final Monitor monitor;
    protected DependencyResolver dependencyResolver;
    protected boolean newInternalStage;
    protected PageCacheArrayFactoryMonitor pageCacheArrayFactoryMonitor;

    // progress of current stage
    protected long goal;
    protected long stashedProgress;
    protected long progress;
    protected ImportStage currentStage;
    protected long lastReportTime;

    public BaseHumanUnderstandableExecutionMonitor( Monitor monitor )
    {
    ...
    public void initialize( DependencyResolver dependencyResolver )
    {
        ...
        BatchingStoreInterface neoStores = dependencyResolver.resolveDependency( BatchingStoreInterface.class );
    ...
    protected static long baselineMemoryRequirement( MemoryStatsVisitor.Visitable neoStores )
    {
        return totalMemoryUsageOf( neoStores );
    }

    protected static long nodesDiskUsage( Input.Estimates estimates, BatchingStoreInterface neoStores )
    {
        return  // node store
                estimates.numberOfNodes() * neoStores.getNodeRecordSize() + //getNodeStore().getRecordSize() +
                        // label index (1 byte per label is not a terrible estimate)
                        estimates.numberOfNodeLabels();
    }

    protected static long relationshipsDiskUsage( Input.Estimates estimates, BatchingStoreInterface neoStores )
    {
        return estimates.numberOfRelationships() * neoStores.getRelationshipRecordSize() *//getRelationshipStore().getRecordSize() *
                (neoStores.usesDoubleRelationshipRecordUnits() ? 2 : 1);
    }


    public boolean tryStart( StageExecution execution )
    {
        // Divide into 4 progress stages:
        if ( execution.getStageName().equals( BaseImportLogic.NODE_IMPORT_NAME ) )
        {
            // Import nodes:
            // - import nodes
            // - prepare id mapper
        ...
        else if ( execution.getStageName().equals( BaseImportLogic.RELATIONSHIP_IMPORT_NAME ) )
        {
            endPrevious();
    ...
    protected void endPrevious()
    {
        updateProgress( goal );
    }

    protected void initializeNodeImport( Input.Estimates estimates, IdMapper idMapper, BatchingStoreInterface neoStores )
    {
    ...
    protected void initializeRelationshipImport( Input.Estimates estimates, IdMapper idMapper, BatchingStoreInterface neoStores )
    {
    ...
    protected void initializeLinking( BatchingStoreInterface neoStores,
                                      NodeRelationshipCache nodeRelationshipCache, DataStatistics distribution )
    {
        ...
        long relationshipRecordIdCount = neoStores.getRelationshipStoreHighId();//getRelationshipStore().getHighId();
    ...
    protected void initializeMisc(BatchingStoreInterface neoStores, DataStatistics distribution )
    {
        ...
        // is that the stages underneath operate on id ranges, not knowing which records are actually in use.
        long relationshipRecordIdCount = neoStores.getRelationshipStoreHighId();//getRelationshipStore().getHighId();
        long groupCount = neoStores.getTemporaryRelationshipGroupStoreHighId();//getTemporaryRelationshipGroupStore().getHighId();
        initializeProgress(
    ...
    protected void initializeProgress( long goal, ImportStage stage )
    {
        this.goal = goal;
    ...
    protected String durationSinceLastReport()
    {
    ...
    protected static int percentage( int line )
    {
        return (line + 1) * PERCENTAGES_PER_LINE;
    }

    protected void printDots( int from, int target )
    {
        int current = from;
    ...
    protected void printPageCacheAllocationWarningIfUsed()
    {
    ...
    protected int dotOf( long progress )
    {
    ...
    protected static int dotsPerLine()
    {
        return DOT_GROUPS_PER_LINE * DOT_GROUP_SIZE;
    }

    protected void printStageHeader( String name, Object... data )
    {
    ...
    /*
    protected static boolean includeStage( StageExecution execution )
    {
     ...
     */
    protected static double weightOf( String stageName )
    {
    ...
    protected static long weighted( String stageName, long progress )
    {
        return (long) (progress * weightOf( stageName ));
    }

    protected static long progressOf( StageExecution execution )
    {
    ...
    protected static Stat findProgressStat( Iterable<Step<?>> steps )
    {
        for ( Step<?> step : steps )
        {
