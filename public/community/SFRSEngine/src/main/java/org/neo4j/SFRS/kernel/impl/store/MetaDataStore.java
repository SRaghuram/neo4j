...
import org.neo4j.SFRS.kernel.impl.store.record.MetaDataRecord;
...
public class MetaDataStore extends CommonAbstractStore<MetaDataRecord,NoStoreHeader> implements MetadataProvider
{
    ...
    public static long setRecord( PageCache pageCache, Path neoStore, Position position, long value, PageCursorTracer cursorTracer ) throws IOException
    {
        ...
        {
            long pageId =  !CommonAbstractStore.isSFRSEngine ? 0: StoreType.META_DATA.fileSFRSStoreRootPage();
            int offset = offset( position );
            try ( PageCursor cursor = pagedFile.io( pageId, PagedFile.PF_SHARED_WRITE_LOCK, cursorTracer ) )
            {
                if ( cursor.next() )
                {
                    ...
                    if ( cursor.checkAndClearBoundsFlag() )
                    {
                        ...
                        throw new UnderlyingStorageException( buildOutOfBoundsExceptionMessage(
                                neoStoreRecord, 0, offset, MetaDataRecordFormat.RECORD_SIZE, pageSize, neoStore.toAbsolutePath().toString() ) );
                    }
                }
            }
        }
    ...
    {
        return MetaDataRecordFormat.RECORD_SIZE * position.id;
    }
    ...
    {
        return getRecord( pageCache, neoStore, position, cursorTracer, false );
    }

    public static long getRecord( PageCache pageCache, Path neoStore, Position position, PageCursorTracer cursorTracer, boolean isSFRSEngine ) throws IOException
    {
        ...
        try ( PagedFile pagedFile = pageCache.map( neoStore, EMPTY, pageSize, immutable.empty() ) )
        {
            ...
            {
                long pageId =  !isSFRSEngine ? 0: StoreType.META_DATA.fileSFRSStoreRootPage();
                try ( PageCursor cursor = pagedFile.io( pageId, PF_SHARED_READ_LOCK, cursorTracer ) )
                {
                    if ( cursor.next() )
                    {
                        ...
                        do
                        {
                            recordFormat.read( record, cursor, RecordLoad.CHECK, MetaDataRecordFormat.RECORD_SIZE, pageSize / MetaDataRecordFormat.RECORD_SIZE );
                            if ( record.inUse() )
                            {
                            ...
                            {
                                value = MetaDataRecordFormat.FIELD_NOT_PRESENT;
                            }
                        }
                        ...
                        if ( cursor.checkAndClearBoundsFlag() )
                        {
                            int offset = offset( position );
                            throw new UnderlyingStorageException( buildOutOfBoundsExceptionMessage(
                                    record, 0, offset, MetaDataRecordFormat.RECORD_SIZE, pageSize, neoStore.toAbsolutePath().toString() ) );
                        }
                    }
                }
            }
    ...
    private long incrementVersion( PageCursor cursor, Position position )
    {
        ...
        checkForDecodingErrors( cursor, position.id, RecordLoad.NORMAL );
        return value;
    }
    ...
    {
        return getRecordValue( cursor, position, MetaDataRecordFormat.FIELD_NOT_PRESENT );
    }
    ...
    private long getRecordValue( PageCursor cursor, Position position, long defaultValue )
    {
        ...
        {
            record.setId( position.id );
            recordFormat.read( record, cursor, RecordLoad.ALWAYS, MetaDataRecordFormat.RECORD_SIZE, getRecordsPerPage() );
            if ( record.inUse() )
            {
