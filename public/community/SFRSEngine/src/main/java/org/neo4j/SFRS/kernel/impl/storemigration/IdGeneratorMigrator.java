...
import org.neo4j.SFRS.kernel.impl.store.StoreFactory;
import org.neo4j.SFRS.kernel.impl.store.StoreType;
...
import static org.neo4j.SFRS.kernel.impl.store.format.RecordStorageCapability.GBPTREE_ID_FILES;
import static org.neo4j.SFRS.kernel.impl.storemigration.FileOperation.MOVE;
...
public class IdGeneratorMigrator extends AbstractStoreMigrationParticipant
{
    ...
    @Override
    public void migrate( org.neo4j.io.layout.DatabaseLayout directoryLayout, org.neo4j.io.layout.DatabaseLayout migrationLayout, ProgressReporter progress, String versionToMigrateFrom,
    ...
    private void migrateIdFiles( org.neo4j.io.layout.DatabaseLayout directoryLayout, org.neo4j.io.layout.DatabaseLayout migrationLayout, RecordFormats oldFormat, RecordFormats newFormat,
            PageCursorTracer cursorTracer ) throws IOException
    {
        ...
        for ( StoreType storeType : StoreType.values() )
        {
            ...
            List<StoreType> list = fileSystem.fileExists( Translate.translate( migrationLayout ).file( storeType.getDatabaseFile() ) )
        ...
        try ( NeoStores stores = createStoreFactory(Translate.translate( directoryLayout ), oldFormat, new ScanOnOpenReadOnlyIdGeneratorFactory() ).openNeoStores(
                storesInDbDirectory.toArray( StoreType[]::new ) ) )
        {
            stores.start( cursorTracer );
            buildIdFiles( Translate.translate( migrationLayout ), storesInDbDirectory, rebuiltIdGenerators, renameList, stores, cursorTracer );
        }
        ...
        Set<Path> placeHolderStoreFiles = createEmptyPlaceHolderStoreFiles( Translate.translate( migrationLayout ), newFormat );
        try ( NeoStores stores = createStoreFactory( Translate.translate( migrationLayout ), newFormat, new ScanOnOpenReadOnlyIdGeneratorFactory() )
                .openNeoStores( storesInMigrationDirectory.toArray( StoreType[]::new ) ) )
        {
            stores.start( cursorTracer );
            buildIdFiles( Translate.translate( migrationLayout ), storesInMigrationDirectory, rebuiltIdGenerators, renameList, stores, cursorTracer );
        }
    ...
    @Override
    public void moveMigratedFiles( org.neo4j.io.layout.DatabaseLayout migrationLayout, org.neo4j.io.layout.DatabaseLayout directoryLayout, String versionToMigrateFrom, String versionToMigrateTo )
            throws IOException
    {
        fileOperation( MOVE, fileSystem, Translate.translate( migrationLayout ), Translate.translate( directoryLayout ),
                Iterables.iterable( DatabaseFile.values() ), true, // allow to skip non existent source files
                ExistingTargetStrategy.OVERWRITE );
    }

    @Override
    public void cleanup( org.neo4j.io.layout.DatabaseLayout migrationLayout )
    {
