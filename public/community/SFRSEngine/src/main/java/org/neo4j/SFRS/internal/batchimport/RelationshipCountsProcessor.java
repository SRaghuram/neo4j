 ...
 */
package org.neo4j.SFRS.internal.batchimport;

import org.neo4j.SFRS.internal.recordstorage.RelationshipCounter;
...
public class RelationshipCountsProcessor implements RecordProcessor<RelationshipRecord>
{
            ...
            NumberArrayFactory cacheFactory, MemoryTracker memoryTracker )
    {
        ...
        this.labelsCounts = cacheFactory.newLongArray( RelationshipCounter.labelsCountsLength( highLabelId, highRelationshipTypeId ), 0, memoryTracker );
        this.wildcardCounts = cacheFactory.newLongArray( RelationshipCounter.wildcardCountsLength( highRelationshipTypeId ), 0, memoryTracker );
        ...
        this.counter = new RelationshipCounter( nodeLabelLookup, highLabelId, highRelationshipTypeId, wildcardCounts, labelsCounts, RelationshipCounter.MANUAL_INCREMENTER );
    }

    static long calculateMemoryUsage( int highLabelId, int highRelationshipTypeId )
    {
        long labelsCountsUsage = RelationshipCounter.labelsCountsLength( highLabelId, highRelationshipTypeId ) * Long.BYTES;
        long wildcardCountsUsage = RelationshipCounter.wildcardCountsLength( highRelationshipTypeId ) * Long.BYTES;
