...
import org.neo4j.SFRS.internal.recordstorage.legacy.IndexCommand;
import org.neo4j.SFRS.internal.recordstorage.legacy.IndexDefineCommand;
import org.neo4j.internal.helpers.collection.Iterables;
import org.neo4j.internal.kernel.api.exceptions.schema.MalformedSchemaRuleException;
import org.neo4j.internal.schema.SchemaRule;
import org.neo4j.io.fs.ReadableChannel;
import org.neo4j.SFRS.kernel.impl.store.AbstractDynamicStore;
import org.neo4j.SFRS.kernel.impl.store.PropertyType;
...
import org.neo4j.SFRS.kernel.impl.store.record.NeoStoreRecord;
...
public class PhysicalLogCommandReaderV3_0_10 extends BaseCommandReader
{
    ...
    private PropertyKeyTokenRecord readPropertyKeyTokenRecord( int id, ReadableChannel channel ) throws IOException
    {
        ...
        record.setNameId( channel.getInt() );
        if ( readDynamicRecords( channel, record, CommandReading.PROPERTY_INDEX_DYNAMIC_RECORD_ADDER ) == -1 )
        {
            return null;
        }
        return record;
    ...
    private Command visitSchemaRuleCommand( ReadableChannel channel ) throws IOException
    {
        ...
        readDynamicRecords( channel, recordsBefore, CommandReading.COLLECTION_DYNAMIC_RECORD_ADDER );
        Collection<DynamicRecord> recordsAfter = new ArrayList<>();
        readDynamicRecords( channel, recordsAfter, CommandReading.COLLECTION_DYNAMIC_RECORD_ADDER );
    ...
    private NodeRecord readNodeRecord( long id, ReadableChannel channel ) throws IOException
    {
        ...
        readDynamicRecords( channel, dynamicLabelRecords, CommandReading.COLLECTION_DYNAMIC_RECORD_ADDER );
    ...
    private PropertyRecord readPropertyRecord( long id, ReadableChannel channel ) throws IOException
    {
        ...
        int deletedRecords = readDynamicRecords( channel, record, CommandReading.PROPERTY_DELETED_DYNAMIC_RECORD_ADDER );
        if ( deletedRecords == -1 )
        {
            return null;
        }
    ...
    private PropertyBlock readPropertyBlock( ReadableChannel channel ) throws IOException
    {
         ...
         */
        if ( readDynamicRecords( channel, toReturn, CommandReading.PROPERTY_BLOCK_DYNAMIC_RECORD_ADDER ) == -1 )
        {
            return null;
        }
        return toReturn;
