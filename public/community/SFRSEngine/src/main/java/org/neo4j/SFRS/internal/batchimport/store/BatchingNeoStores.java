 ...
 */
package org.neo4j.SFRS.internal.batchimport.store;

import org.eclipse.collections.api.set.ImmutableSet;
import org.neo4j.SFRS.internal.batchimport.AdditionalInitialIds;
import org.neo4j.SFRS.internal.batchimport.store.BatchingTokenRepository.BatchingLabelTokenRepository;
import org.neo4j.SFRS.internal.batchimport.store.BatchingTokenRepository.BatchingPropertyKeyTokenRepository;
import org.neo4j.SFRS.internal.batchimport.store.BatchingTokenRepository.BatchingRelationshipTypeTokenRepository;
import org.neo4j.SFRS.internal.batchimport.store.io.IoTracer;
import org.neo4j.SFRS.internal.counts.CountsBuilder;
...
import static org.neo4j.SFRS.kernel.impl.store.StoreType.*;
...
public class BatchingNeoStores implements AutoCloseable, MemoryStatsVisitor.Visitable
{
            ...
            AdditionalInitialIds initialIds, boolean externalPageCache, IoTracer ioTracer, PageCacheTracer pageCacheTracer, MemoryTracker memoryTracker )
    {
        ...
        if (neo4jConfig.isSFRSEngine( databaseLayout.getDatabaseName()) || neo4jConfig.isOneIDFile( databaseLayout.getDatabaseName()))
    ...
    private void instantiateExtensions()
    {
        ...
        life.start();
        labelScanStore = TokenScanStore.labelScanStore( pageCache, Translate.translate(databaseLayout), fileSystem, EMPTY, false, new Monitors(), immediate(),
                pageCacheTracer, memoryTracker );
        life.add( labelScanStore );
        relationshipTypeScanStore = TokenScanStore
                .toggledRelationshipTypeScanStore( pageCache, Translate.translate( databaseLayout ), fileSystem, EMPTY, false, new Monitors(), immediate(), neo4jConfig,
                        pageCacheTracer, memoryTracker );
    ...
    public void close() throws IOException
    {
        ...
        try {
            ...
            if (idGeneratorFactory instanceof DefaultIdGeneratorFactory)
                if (((DefaultIdGeneratorFactory)idGeneratorFactory).getGBPTree() != null)
                    ((DefaultIdGeneratorFactory)idGeneratorFactory).getGBPTree().close();
        } catch (IOException e)
        {
             System.out.println("Could not close GBT Tree");
        }
