...
import org.neo4j.SFRS.kernel.impl.store.format.RecordFormatSelector;
...
import org.neo4j.SFRS.kernel.impl.storemigration.IdGeneratorMigrator;
import org.neo4j.SFRS.kernel.impl.storemigration.RecordStorageMigrator;
import org.neo4j.SFRS.kernel.impl.storemigration.RecordStoreVersion;
import org.neo4j.SFRS.kernel.impl.storemigration.RecordStoreVersionCheck;
...
import static org.neo4j.SFRS.kernel.impl.store.StoreType.META_DATA;
import static org.neo4j.SFRS.kernel.impl.store.format.RecordFormatSelector.selectForStoreOrConfig;
import static org.neo4j.SFRS.kernel.impl.store.format.RecordFormatSelector.selectForVersion;
import static org.neo4j.index.internal.gbptree.RecoveryCleanupWorkCollector.immediate;

@ServiceProvider
public class SFRSStorageEngineFactory implements StorageEngineFactory
{
    @Override
    public StoreVersionCheck versionCheck( FileSystemAbstraction fs, org.neo4j.io.layout.DatabaseLayout databaseLayout, Config config, PageCache pageCache,
            LogService logService, PageCacheTracer pageCacheTracer )
    {
        return new RecordStoreVersionCheck( fs, pageCache,
                Translate.translate( databaseLayout, config.isSFRSEngine( databaseLayout.getDatabaseName()), config.isOneIDFile( databaseLayout.getDatabaseName()) ),
                logService.getInternalLogProvider(), config, pageCacheTracer );
    }
    ...
    @Override
    public StorageEngine instantiate( FileSystemAbstraction fs, org.neo4j.io.layout.DatabaseLayout databaseLayout, Config config, PageCache pageCache, TokenHolders tokenHolders,
    ...
    {
        return new RecordStorageEngine( Translate.translate( databaseLayout,
                config.isSFRSEngine( databaseLayout.getDatabaseName()), config.isOneIDFile( databaseLayout.getDatabaseName()) ),
                config, pageCache, fs, logProvider, tokenHolders, schemaState, constraintSemantics,
    ...
    @Override
    public List<Path> listStorageFiles( FileSystemAbstraction fileSystem, org.neo4j.io.layout.DatabaseLayout databaseLayout ) throws IOException
    {
        ...
        return Arrays.stream( StoreType.values() )
                     .map( t -> Translate.translate( databaseLayout ).file( t.getDatabaseFile() ) )
                     .filter( fileSystem::fileExists ).collect( toList() );
    }

    @Override
    public boolean storageExists( FileSystemAbstraction fileSystem, org.neo4j.io.layout.DatabaseLayout databaseLayout, PageCache pageCache, Config config )
    {
        if (config.isSFRSEngine(databaseLayout.getDatabaseName()))
            return true;
        return false;
        //return NeoStores.isStorePresent( fileSystem, Translate.translate( databaseLayout ) );
    }

    @Override
    public TransactionIdStore readOnlyTransactionIdStore( FileSystemAbstraction fileSystem, org.neo4j.io.layout.DatabaseLayout databaseLayout, PageCache pageCache,
            PageCursorTracer cursorTracer ) throws IOException
    {
        return new ReadOnlyTransactionIdStore( fileSystem, pageCache, Translate.translate( databaseLayout ), cursorTracer );
    }

    @Override
    public LogVersionRepository readOnlyLogVersionRepository( org.neo4j.io.layout.DatabaseLayout databaseLayout, PageCache pageCache, PageCursorTracer cursorTracer )
            throws IOException
    {
    ...
    @Override
    public MetadataProvider transactionMetaDataStore( FileSystemAbstraction fs, org.neo4j.io.layout.DatabaseLayout databaseLayout, Config config, PageCache pageCache,
            PageCacheTracer cacheTracer )
    {
        RecordFormats recordFormats = selectForStoreOrConfig( Config.defaults(), Translate.translate( databaseLayout ), fs, pageCache, NullLogProvider.getInstance(), cacheTracer );
        ...
        return new StoreFactory( Translate.translate( databaseLayout,
                config.isSFRSEngine( databaseLayout.getDatabaseName()), config.isOneIDFile( databaseLayout.getDatabaseName()) ), config, factory, pageCache, fs, recordFormats, NullLogProvider.getInstance(), cacheTracer, immutable.empty() )
                .openNeoStores( META_DATA ).getMetaDataStore();
    }

    @Override
    public StoreId storeId( org.neo4j.io.layout.DatabaseLayout databaseLayout, PageCache pageCache, PageCursorTracer cursorTracer ) throws IOException
    {
        return MetaDataStore.getStoreId( pageCache, databaseLayout.metadataStore(), cursorTracer );
    }

    @Override
    public SchemaRuleMigrationAccess schemaRuleMigrationAccess( FileSystemAbstraction fs, PageCache pageCache, Config config, org.neo4j.io.layout.DatabaseLayout databaseLayout,
            LogService logService, String recordFormats, PageCacheTracer cacheTracer, PageCursorTracer cursorTracer, MemoryTracker memoryTracker )
    {
        ...
        StoreFactory factory = new StoreFactory( Translate.translate( databaseLayout ), config, new DefaultIdGeneratorFactory( fs, immediate() ), pageCache, fs, formats,
    ...
    @Override
    public StorageFilesState checkRecoveryRequired( FileSystemAbstraction fs, org.neo4j.io.layout.DatabaseLayout databaseLayout, PageCache pageCache )
    {
