...
import static org.neo4j.SFRS.kernel.impl.store.NodeLabelsField.parseLabelsBody;
...
public class InlineNodeLabels implements NodeLabels
{
            ...
            MemoryTracker memoryTracker )
    {
        long[] augmentedLabelIds = labelCount( node.getLabelField() ) == 0 ? new long[]{labelId} :
                                   LabelIdArray.concatAndSort( parseInlined( node.getLabelField() ), labelId );
    ...
    {
        long[] newLabelIds = LabelIdArray.filter( parseInlined( node.getLabelField() ), labelId );
