...
import org.neo4j.SFRS.kernel.impl.store.format.RecordFormat;
        ...
        implements RecordStore<RECORD>, AutoCloseable
{
    ...
    private boolean checkAndLoadStorage( boolean createIfNotExists, PageCursorTracer cursorTracer )
    {
        try
        {
            ...
            if ( getNumberOfReservedLowIds() > 0 )
            {
                ...
                if (isSFRSEngine() && parentNeoStores.newStoreCreation) {
                    throw new NoSuchFileException("SFRS ENgine");
                }
        ...
        {
            parentNeoStores.newStoreCreation = true;
            if ( pagedFile != null )
            {
                pagedFile.close();
                pagedFile = null;
    ...
    {
        long pageId = !isSFRSEngine() ? 0: storeType.fileSFRSStoreRootPage();
        if ( getNumberOfReservedLowIds() > 0 )
        {
            try ( PageCursor pageCursor = pagedFile.io( pageId, PF_SHARED_WRITE_LOCK | PF_EAGER_FLUSH, cursorTracer ) )
            {
                if ( pageCursor.next() )
                {
        ...
        if (isSFRSEngine() && storeType.isExtensibleStore())
            initPBTablePage( pageId );
    }
    ...
    {
        long pageId = !isSFRSEngine() ? 0: storeType.fileSFRSStoreRootPage();
        try ( PageCursor pageCursor = pagedFile.io( pageId, PF_SHARED_READ_LOCK, cursorTracer ) )
        {
            HEADER readHeader;
            if ( pageCursor.next() )
            {
    ...
    protected long pageIdForRecord( long id )
    {
        ...
        return pageIdForRecord( id, false);
    }

    protected long pageIdForRecord( long id, boolean allocate )
    {
        int filePageSize = isSFRSEngine() ?
                pageCache.pageSize() - pageCache.pageSize() % getRecordSize():
                pagedFile.pageSize();
        long pageId = RecordPageLocationCalculator.pageIdForRecord( id, filePageSize, recordSize );
        if (isSFRSEngine()) {
            //do {
            if (!storeType.isExtensibleStore()) {
                //this is only for stores that are fixed in size and not expanding type
                return storeType.fileSFRSStoreRootPage() + RecordPageLocationCalculator.pageIdForRecord(id, filePageSize, recordSize);
            }
            //long maxId = idGenerator.getHighId();
            //maxAllocatedID = maxAllocatedID < maxId ? maxId : maxAllocatedID;
            if (!allocate && id > maxAllocatedID) {
                //throw exception
                //throwOutOfBoundsException( id );
            }
            if (allocate)
                //while (maxAllocatedID < maxId || maxAllocatedID < id)
                synchronized(maxAllocatedID) {
                    while (maxAllocatedID.longValue() <= id) {
                        //allocate up to maxId and id
                        maxAllocatedID = allocateNextEntryInPBTable(maxAllocatedID);
                    }
                }
            pageId = virtual2ActualPageId(pageId);//getPageIdSFRS( id );
            //printMessage("Id2PageId:[" + storeType.name()+"]["+storeType.fileSFRSStoreRootPage()+"]-["+ id + "][" + pageId +"]");
            //} while (checkPageIdForStore( pageId ));
        }
        return pageId;
    }
    ...
    protected int offsetForId( long id )
    {
        if (isSFRSEngine()) {
            ...
            //if (!storeType.isExtensibleStore())
            //    return Integer.BYTES + RecordPageLocationCalculator.offsetForId( id, filePageSize, recordSize );
            return RecordPageLocationCalculator.offsetForId( id, recordSize, filePageSize / recordSize );
        }
        return RecordPageLocationCalculator.offsetForId( id, recordSize, pageCache.pageSize() / recordSize );
    }

    @Override
    public int getRecordsPerPage()
    {
        if (recordSize == 0 || recordSize > 128)
            recordSize = determineRecordSize();
    ...
    public byte[] getRawRecordData( long id, PageCursorTracer cursorTracer ) throws IOException
    {
        ...
        try ( PageCursor cursor = pagedFile.io( pageId, PagedFile.PF_SHARED_READ_LOCK, cursorTracer ) )
        {
            if ( cursor.next() )
            {
                ...
                checkForDecodingErrors( cursor, id, RecordLoad.FORCE ); // Clear errors from the cursor.
            }
        }
        return data;
    }
    ...
    public boolean isInUse( long id, PageCursorTracer cursorTracer )
    {
        ...
        try ( PageCursor cursor = pagedFile.io( pageId, PF_SHARED_READ_LOCK, cursorTracer ) )
        {
            ...
            if ( cursor.next() )
            {
                ...
                while ( cursor.shouldRetry() );
                checkForDecodingErrors( cursor, id, RecordLoad.NORMAL );
            }
    ...
    private FreeIds freeIds( PageCursorTracer cursorTracer )
    {
        ...
        {
            long pageId = !isSFRSEngine() ? 0: storeType.fileSFRSStoreRootPage();
            try ( PageCursor cursor = pagedFile.io( pageId, PF_SHARED_READ_LOCK | PF_READ_AHEAD, cursorTracer ) )
            {
                ...
                long foundHighId = (!isSFRSEngine())? scanForHighId( cursorTracer ) : scanForHighIdSFRS( cursorTracer );
    ...
    {
        boolean readOnly = configuration.get( GraphDatabaseSettings.read_only );
        idGenerator = idGeneratorFactory.open( pageCache, idFile, getIdType(),
                () -> (!isSFRSEngine())? scanForHighId( cursorTracer ) : scanForHighIdSFRS( cursorTracer ),
                recordFormat.getMaxId(), readOnly, cursorTracer, openOptions );
    }
    ...
    protected long scanForHighIdSFRS( PageCursorTracer cursorTracer )
    {
        boolean found = false;
        long highestId = getNumberOfReservedLowIds();
        long lastPageBlock = findLastPageBlock( cursorTracer );
        try ( PageCursor cursor = pagedFile.io( lastPageBlock, PF_SHARED_READ_LOCK, cursorTracer ) )
        {
            int recordsPerPage = getRecordsPerPage();
            int recordSize = getRecordSize();
            /*
             ...
             * We first pre-fetch a large chunk sequentially, which is then scanned backwards for used records.
             */
            final long chunkSizeInPages = DatabaseLayout.PAGEBLOCK_SIZE; // 2MiB (8192 bytes/page * 256 pages/chunk)

            long chunkStartId = lastPageBlock;
            long chunkEndId = !storeType.isExtensibleStore() ? lastPageBlock : lastPageBlock + chunkSizeInPages -1;
            long startId = chunkStartId, endId = chunkEndId;
            while ( chunkEndId >=  chunkStartId)
            {
                // Do pre-fetch of the chunk
                preFetchChunk( cursor, chunkStartId, chunkEndId );
                ...
                for ( long currentId = chunkEndId; currentId >= chunkStartId && cursor.next( currentId ); currentId-- )
                {
                    do
                    {
                        ...
                        for ( int offset = recordsPerPage * recordSize - recordSize; offset >= 0; offset -= recordSize )
                        {
                            ...
                            {
                                // We've found the highest id in use
                                long currentVirtualPageId = actual2VirtualPageId( cursor.getCurrentPageId(), cursorTracer );
                                highestId = (currentVirtualPageId * recordsPerPage) + offset / recordSize + 1;
                                found = true;
                                printMessage("ScanforHighId Found["+storeType.name()+"]:["+highestId +"]["+ startId+":"+endId+"]");
                                break;
                            }
                        }
                    }
            ...
            printMessage("ScanforHighId NOT Found["+storeType.name()+"]:["+highestId +"]["+ startId+":"+endId+"]");
            return getNumberOfReservedLowIds();
        }
        catch ( IOException e )
        {
    ...
    {
        return idGenerator != null ? idGenerator.getHighestPossibleIdInUse() : ((!isSFRSEngine())? scanForHighId( cursorTracer ) -1  : scanForHighIdSFRS( cursorTracer ) - 1);
    }
    ...
    public void updateRecord( RECORD record, IdUpdateListener idUpdateListener, PageCursorTracer cursorTracer )
    {
        ...
        long pageId = pageIdForRecord( id, true );
        if (storeType == StoreType.PROPERTY && pageId == 0 && !pagedFile.path().toString().contains(".id") && !pagedFile.path().toString().contains(".db") &&
                !pagedFile.path().toString().contains("temp")) {
            //System.out.println(this.pagedFile.path() + ":");
            pageId = pageIdForRecord( id );
        }
        int offset = offsetForId( id );
        if (isSFRSEngine() && parentNeoStores.pbTablePages.contains( pageId )) {
            //System.out.println("Writing PBTablePage[" + storeType.name() + "] [" + pageId + ":" + offset + "]");
        }
        try ( PageCursor cursor = pagedFile.io( pageId, PF_SHARED_WRITE_LOCK, cursorTracer ) )
        {
            if ( cursor.next() )
            {
                ...
                if (storeType.name().equals( StoreType.SCHEMA.name()) )
                    printMessage("Write["+storeType.name()+"]["+record.toString()+"]");
                checkForDecodingErrors( cursor, id, RecordLoad.NORMAL ); // We don't free ids if something weird goes wrong
                if ( !record.inUse() )
                {
    ...
    {
        long pageId = !isSFRSEngine() ? 0: storeType.fileSFRSStoreRootPage();
        try ( PageCursor cursor = openPageCursorForReading( pageId, cursorTracer ) )
        {
            RECORD record = newRecord();
            long highId = getHighId();
            for ( long id = getNumberOfReservedLowIds(); id < highId; id++ )
            {
                getRecordByCursor( id, record, RecordLoad.CHECK, cursor );
                if ( record.inUse() )
                {
    ...
    private static boolean creationInProgress = false;

    private ArrayList<Integer> PBList = new ArrayList<Integer>();
    long recordsInPB = 0;
    long recordsInFirstPB = 0;

    long firstPBTablePages = 0;
    long restPBTablePages = 0;

    int numPBEntriesInPBTablePage = 0;
    int numPBEntriesInFirstPBTablePage = 0 ;

    int numPagesInPBTablePage = 0;
    int numPagesInFirstPBTablePage = 0;

    Long maxAllocatedID = new Long(0l );
    public static boolean isSFRSEngine = false;
    ...
    public void setContext( NeoStores neoStores, StoreType storeType, DatabaseLayout layout)
    {
        ...
        this.layout = layout;
        isSFRSEngine = layout.isSFRSEngine();
    }

    private boolean isSFRSEngine()
    {
        return layout != null && layout.isSFRSEngine();
    }

    private void initSFRSValues()
    {
        if (!isSFRSEngine())
            return;
        recordsPerPage = getRecordsPerPage();
        buildPBTable();
        recordsInPB = recordsPerPage * DatabaseLayout.PAGEBLOCK_SIZE;
        recordsInFirstPB = recordsPerPage * (DatabaseLayout.PAGEBLOCK_SIZE - 1);

        int maxEntries = PAGE_SIZE/ DatabaseLayout.PBTABLE_ENTRY_SIZE;
        numPBEntriesInPBTablePage = maxEntries - ((DatabaseLayout.PBTABLE_HEADER_SIZE/DatabaseLayout.PBTABLE_ENTRY_SIZE) + 1);
        numPBEntriesInFirstPBTablePage = numPBEntriesInPBTablePage - (DatabaseLayout.PBTABLE_INITIAL_OFFSET/DatabaseLayout.PBTABLE_ENTRY_SIZE) ;

        numPagesInPBTablePage = ((numPBEntriesInPBTablePage + 1) * DatabaseLayout.PAGEBLOCK_SIZE) - 1;
        numPagesInFirstPBTablePage = ((numPBEntriesInFirstPBTablePage + 1 ) * DatabaseLayout.PAGEBLOCK_SIZE) - 1;

        // the subtraction of recordsPerPage is for 1 page reserved for PageBlockTable.
        maxAllocatedID = recordsInFirstPB;
        firstPBTablePages = numPBEntriesInFirstPBTablePage * DatabaseLayout.PAGEBLOCK_SIZE
                + DatabaseLayout.PAGEBLOCK_SIZE - 1;
        restPBTablePages = numPBEntriesInPBTablePage * DatabaseLayout.PAGEBLOCK_SIZE
                + DatabaseLayout.PAGEBLOCK_SIZE - 1;
    }

    public static void printMessage(String msg)
    {
        //if (IdContainer.printMessages)
            //System.out.println(msg);
    }

    private void buildPBTable()
    {
        if (!storeType.isExtensibleStore())
            return;
        long pageId = storeType.fileSFRSStoreRootPage();
        StringBuilder list = new StringBuilder();
        try ( PageCursor pageCursor = pagedFile.io( pageId, PF_SHARED_WRITE_LOCK, PageCursorTracer.NULL ) )
        {
            do {
                PBList.add( (int) pageId );
                list.append( pageId +",");
                if ( pageCursor.next( pageId ) )
                {
                    do {
                        pageCursor.setOffset(PAGE_SIZE - DatabaseLayout.PBTABLE_ENTRY_SIZE);
                        pageId = pageCursor.getInt();
                    } while ( pageCursor.shouldRetry() );
                }
                else {
                    throw new StoreNotFoundException( "Fail to getBlockIdFromPBTable of store file: " + storageFile );
                }

            } while (pageId != 0);
            int lastEntryOffset = pageId == storeType.fileSFRSStoreRootPage() ? DatabaseLayout.PBTABLE_INITIAL_OFFSET : 0;
            long lastPB = pageCursor.getShort( lastEntryOffset );
            printMessage("Build PBList for ["+ storeType.name()+"]["+list.toString()+ "Last Index ["+ lastPB +"]]");
        } catch (IOException e) {
            printMessage("TBD: No added code to handle IO exception");
            e.printStackTrace();
            //TBD
        }
    }

    private long virtual2ActualPageId( long vPageId)
    {
        long pbTablePageIndex = vPageId < firstPBTablePages ? 0 :
                1 + (vPageId - firstPBTablePages)/restPBTablePages;
        long pbTablePageOffet = vPageId < firstPBTablePages ? vPageId :
                (vPageId - firstPBTablePages) % restPBTablePages;
        long pbTableEntryIndex = pbTablePageOffet < (DatabaseLayout.PAGEBLOCK_SIZE - 1) ? -1 :
                (pbTablePageIndex == 0 ? (DatabaseLayout.PBTABLE_INITIAL_OFFSET/DatabaseLayout.PBTABLE_ENTRY_SIZE) : 0) +
                        (DatabaseLayout.PBTABLE_HEADER_SIZE/DatabaseLayout.PBTABLE_ENTRY_SIZE) +
                        (pbTablePageOffet - (DatabaseLayout.PAGEBLOCK_SIZE - 1)) / DatabaseLayout.PAGEBLOCK_SIZE;
        //use pbTableEntryIndex to get pbPageId from pbTablePage obtained using pbTablePageIndex
        if (pbTablePageIndex >= PBList.size()) {
            printMessage("Error");
        }
        long pbTablePageId = PBList.get( (int) pbTablePageIndex );
        long pbPageId = pbTableEntryIndex == -1 ? pbTablePageId + 1 : getPageIdFromPBTable( pbTablePageId, pbTableEntryIndex);
        long pbOffset = pbTablePageOffet < (DatabaseLayout.PAGEBLOCK_SIZE - 1) ? pbTablePageOffet :
                (pbTablePageOffet  - (DatabaseLayout.PAGEBLOCK_SIZE - 1)) % DatabaseLayout.PAGEBLOCK_SIZE;
        long actualPageId = pbPageId + pbOffset;
        return actualPageId;
    }

    private long allocateNextEntryInPBTable( long currentMax ) {
        int pageId = PBList.get(PBList.size()-1);
        try (PageCursor pageCursor = pagedFile.io(pageId, PF_SHARED_WRITE_LOCK, PageCursorTracer.NULL )) {
            int value = -1;
            int index = 0;
            int offset = PBList.size() == 1 ? DatabaseLayout.PBTABLE_INITIAL_OFFSET + DatabaseLayout.PBTABLE_HEADER_SIZE:
                    DatabaseLayout.PBTABLE_INITIAL_OFFSET  ;
            if (pageCursor.next()) {
                do {

                    do {
                        value = pageCursor.getInt( offset );
                    }
                    while (pageCursor.shouldRetry());
                    if (value == 0) {
                        int nextPBId = DatabaseLayout.getNextPDBId();
                        pageCursor.putInt(offset, nextPBId);
                        int lastEntryOffset = (pageId == storeType.fileSFRSStoreRootPage()) ? DatabaseLayout.PBTABLE_INITIAL_OFFSET : 0;
                        pageCursor.putShort(lastEntryOffset, (short)offset);
                        // if this happens to last entry in this page of PBTable then the
                        //  allocated page block will have the next page of the PBTable.
                        if ((offset + DatabaseLayout.PBTABLE_ENTRY_SIZE) >= PAGE_SIZE)
                        {
                            printMessage("\n["+storeType.name()+"] PBList add page["+nextPBId+":"+(currentMax+recordsInFirstPB)+"] as ["+ (PBList.size()+1)+"] entry");
                            PBList.add( nextPBId );
                            initPBTablePage( nextPBId );
                            return currentMax+recordsInFirstPB;
                        }
                        //printMessage("["+storeType.name()+"] Adding a Pageblock at index["+index+"] page ["+nextPBId+"]");
                        return currentMax+recordsInPB;
                    }
                    offset += DatabaseLayout.PBTABLE_ENTRY_SIZE;
                    index++;
                } while (offset < PAGE_SIZE);
            } else {
                throw new StoreNotFoundException("Fail to getNumEntriesPBTable of store file: " + storageFile);
            }
        } catch (IOException e) {
            printMessage("TBD: getNumEntriesPBTable - No added code to handle IO exception");
            e.printStackTrace();
            //TBD
            return -1;
        }
    ...
    private int getPageIdFromPBTable( long pageId, long index)
    {
        try ( PageCursor pageCursor = pagedFile.io( pageId, PF_SHARED_READ_LOCK, PageCursorTracer.NULL ) )
        {
            ...
            if ( pageCursor.next() )
            {
                do
                {
                    pageCursor.setOffset((int) (DatabaseLayout.PBTABLE_ENTRY_SIZE * index));
                    return pageCursor.getInt();
                }
                while ( pageCursor.shouldRetry() );
            }
            else
    ...
    private boolean initPBTablePage(long pageId ) {

        if (parentNeoStores.pbTablePages.contains( pageId ))
            printMessage("ERROR in pageBlockTablePage:"+ pageId);
        try (PageCursor pageCursor = pagedFile.io(pageId, PF_SHARED_WRITE_LOCK, PageCursorTracer.NULL)) {
            if (pageCursor.next()) {
                //aviod overwriting the header in the root page
                int offset = pageId == storeType.fileSFRSStoreRootPage() ? DatabaseLayout.PBTABLE_INITIAL_OFFSET : 0;
                while ((offset + DatabaseLayout.PBTABLE_ENTRY_SIZE) <= PAGE_SIZE) {
                    do {
                        pageCursor.putInt(offset, 0);
                        offset += DatabaseLayout.PBTABLE_ENTRY_SIZE;
                    }
                    while (pageCursor.shouldRetry());
                }
            } else {
                throw new StoreNotFoundException("Fail to init PBTable of store file: " + storageFile);
            }
        } catch (IOException e) {
            printMessage("TBD: No added code to handle IO exception");
            e.printStackTrace();
        ...
        parentNeoStores.pbTablePages.add( pageId );
        printMessage("PBTable Page Init------:"+parentNeoStores.pbTablePages.toString());
        return true;
    }

    private long actual2VirtualPageId( long aPageId, PageCursorTracer cursorTracer)
    {
        if (!storeType.isExtensibleStore())
            return 0;
        long vPageId = 0, pageId = -1;
        for (int listIndex = 0; listIndex < PBList.size(); listIndex++)
        {
            pageId = PBList.get( listIndex );
            int headerOffset = (listIndex == 0 ? DatabaseLayout.PBTABLE_INITIAL_OFFSET : 0) +
                    (DatabaseLayout.PBTABLE_HEADER_SIZE);
            if (aPageId > pageId && aPageId <= pageId + DatabaseLayout.PAGEBLOCK_SIZE)
                return vPageId + (aPageId - pageId - 1 );
            int maxIndex = listIndex == 0 ? numPBEntriesInFirstPBTablePage : numPBEntriesInPBTablePage;
            try ( PageCursor pageCursor = pagedFile.io( pageId, PF_SHARED_READ_LOCK, cursorTracer ) )
            {
                if ( pageCursor.next() )
                {
                    int entry = -1;
                    for (int entryIndex = 0; entryIndex < maxIndex && entry != 0; entryIndex++) {
                        do {
                            pageCursor.setOffset((int) (headerOffset + DatabaseLayout.PBTABLE_ENTRY_SIZE * entryIndex));
                            entry = pageCursor.getInt();
                            if (aPageId >= entry && aPageId < entry + DatabaseLayout.PAGEBLOCK_SIZE)
                                return vPageId + (aPageId - entry);
                        }
                        while (pageCursor.shouldRetry());
                        vPageId += DatabaseLayout.PAGEBLOCK_SIZE;
                    }
                }
                else
                {
    ...
    private long findLastPageBlock( PageCursorTracer cursorTracer ) {
        long pageId = storeType.fileSFRSStoreRootPage();
        if (!storeType.isExtensibleStore())
            return pageId;
        try (PageCursor pageCursor = pagedFile.io(pageId, PF_SHARED_READ_LOCK, cursorTracer)) {
            ...
            if (pageCursor.next()) {
                while (true)
                {
                    int offset = 0;
                    do {
                        offset = (firstPB ? DatabaseLayout.PBTABLE_INITIAL_OFFSET + DatabaseLayout.PBTABLE_HEADER_SIZE : 0)
                                + DatabaseLayout.PBTABLE_PAGE_HEADER_SIZE
                                + DatabaseLayout.PBTABLE_ENTRY_SIZE * index;
                        value = pageCursor.getInt( offset );
                    }
                    while (pageCursor.shouldRetry());
                    if (value == 0) {
                        if (index == 0)
                            return pageId;
                        return pageCursor.getInt(offset - DatabaseLayout.PBTABLE_ENTRY_SIZE);
                    }
                    if (index == numPBEntriesInPBTablePage -1)
                    {
                        //go to thw next PB using the current (last) entry
                        firstPB = false;
                        pageId = value;
