...
import org.neo4j.SFRS.internal.batchimport.stats.Keys;

import static org.junit.jupiter.api.Assertions.assertEquals;
import static org.mockito.Mockito.never;
import static org.mockito.Mockito.spy;
import static org.mockito.Mockito.verify;
import static org.neo4j.SFRS.internal.batchimport.staging.ControlledStep.stepWithStats;
...
class DynamicProcessorAssignerTest
{
    ...
    void shouldRemoveCPUsFromWayTooFastStep()
    {
        ...
        ControlledStep<?> slowStep = Mockito.spy( ControlledStep.stepWithStats( "slow", 1, Keys.avg_processing_time, 6L, Keys.done_batches, 10L )
                .setProcessors( 2 ) );
        ControlledStep<?> fastStep = Mockito.spy( ControlledStep.stepWithStats( "fast", 0, Keys.avg_processing_time, 2L, Keys.done_batches, 10L )
                .setProcessors( 2 ) );
    ...
    void shouldRemoveCPUsButNotSoThatTheFastStepBecomesBottleneck()
    {
        ...
        ControlledStep<?> slowStep = Mockito.spy( ControlledStep.stepWithStats( "slow", 1, Keys.avg_processing_time, 10L, Keys.done_batches, 10L ) );
        ControlledStep<?> fastStep = Mockito.spy( ControlledStep.stepWithStats( "fast", 0, Keys.avg_processing_time, 7L, Keys.done_batches, 10L )
                .setProcessors( 3 ) );
    ...
    void shouldRemoveCPUsFromTooFastStepEvenIfThereIsAWayFaster()
    {
        ...
        Configuration config = config( 10, 3 );
        DynamicProcessorAssigner assigner = new DynamicProcessorAssigner( config );
        Step<?> wayFastest = ControlledStep.stepWithStats( "wayFastest", 0, Keys.avg_processing_time, 50L, Keys.done_batches, 20L );
        Step<?> fast = Mockito.spy( ControlledStep.stepWithStats( "fast", 0, Keys.avg_processing_time, 100L, Keys.done_batches, 20L )
                .setProcessors( 3 ) );
        Step<?> slow = ControlledStep.stepWithStats( "slow", 1, Keys.avg_processing_time, 220L, Keys.done_batches, 20L );
    ...
    void shouldRemoveCPUsFromTooFastStepEvenIfNotAllPermitsAreUsed()
    {
        ...
        DynamicProcessorAssigner assigner = new DynamicProcessorAssigner( config );
        Step<?> wayFastest = Mockito.spy( ControlledStep.stepWithStats( "wayFastest", 0,
                Keys.avg_processing_time, 50L,
                Keys.done_batches, 20L )
                .setProcessors( 5 ) );
        Step<?> fast = Mockito.spy( ControlledStep.stepWithStats( "fast", 0,
                Keys.avg_processing_time, 100L,
                Keys.done_batches, 20L )
                .setProcessors( 3 ) );
        Step<?> slow = ControlledStep.stepWithStats( "slow", 1,
